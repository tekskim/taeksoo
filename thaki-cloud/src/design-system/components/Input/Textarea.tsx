    ...
    return (
      <div className={wrapperClasses}>
        {label && (
          <label
            htmlFor={inputId}
            className="font-medium text-[var(--color-text-default)] text-[length:var(--font-size-12)]"
          >
            {label}
          </label>
        )}
        ...
        <div className="flex items-center justify-between">
          <div>
            {error && (
              <p id={`${inputId}-error`} className="text-[length:var(--font-size-12)] text-[var(--color-state-danger)]">
                {error}
              </p>
            )}

            {helperText && !error && (
              <p id={`${inputId}-helper`} className="text-[length:var(--font-size-12)] text-[var(--color-text-subtle)]">
                {helperText}
              </p>
            )}
          </div>
