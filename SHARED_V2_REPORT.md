...
Create 페이지(Container 39개, Compute 7개 등)는 shared 컴포넌트로 구현되었으나, 일부 컴포넌트/API 차이로 TDS와 동일한 수준의 구현이 어려운 부분이 있습니다.

| 영역                  | 현재 상태                                   | TDS 기준                              |
| --------------------- | ------------------------------------------- | ------------------------------------- |
| **Slider**            | `NumberInput`만 사용 (Replicas, Scaling 등) | `Slider` + `NumberInput` 조합         |
| **Select**            | 호환 래퍼(`TdsSelectCompat`) 사용           | `Select options={[...]}` (배열 props) |
| **RadioGroup**        | 호환 래퍼(`TdsRadioCompat`) 사용            | `RadioGroup` + `Radio` (Context 기반) |
| **Summary 사이드바**  | `FloatingCard` (flat 상태 표시)             | 계층적 상태 + 섹션별 진행률           |
| **FilterSearchInput** | API 차이로 필터 동작 미연결                 | 필터 상태 연동                        |
| **앱 쉘**             | `CreateLayout`만 사용                       | `PageShell` + `TopBar` + `TabBar`     |
| **Chip**              | `Tag`로 대체                                | `Chip` 별도 컴포넌트                  |

### Dynamic Form Fields

TDS에 8가지 동적 폼 패턴(Key-Value, Multi-Column Ports, Key-Operator-Value 등)이 문서화되어 있으나, 현재 TDS/shared 양쪽 모두 재사용 컴포넌트가 아닌 인라인 grid로 반복 구현하고 있습니다.

| 패턴               | 사용 예                       | 현재 구현                                      |
| ------------------ | ----------------------------- | ---------------------------------------------- |
| Key-Value          | Labels, Annotations, Env vars | `grid-cols-[1fr_1fr_20px]` 인라인 반복         |
| Multi-Column       | Ports, Volumes, Mount paths   | `grid-cols-[1fr_1fr_1fr_1fr_20px]` 인라인 반복 |
| Key-Operator-Value | Node Selector, Pod Affinity   | `grid-cols-[1fr_140px_1fr_20px]` 인라인 반복   |
| Single-Value List  | Nameservers, Search Domains   | `grid-cols-[1fr_20px]` 인라인 반복             |

이 패턴들을 컴포넌트로 추출하면 Create 페이지 전반의 코드 품질과 일관성이 개선됩니다.
