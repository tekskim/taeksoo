...
| ---------------------------- | --------------------------------------- | -------------------------------------------- | --------------------------------------------- | ------------------------------ |
| **컴포넌트 구조**            | 단일 Input 컴포넌트                     | 별도 Textarea 컴포넌트                       | Input + Textarea (같은 폴더에서 export)       | ✅ 호환                        |
| **size**                     | `'xs' \| 'sm' \| 'md' \| 'lg'`          | `'sm' \| 'md' \| 'lg'`                       | `'sm' \| 'md'` (호환 alias: `'xs'` \| `'lg'`) | ✅ 호환 (alias 지원)           |
...
| **Textarea 전용**            | -                                       | `resize`, `autoResize`, `showCharacterCount` | tds Textarea 별도 존재                        | ✅ 호환                        |

**주요 차이점:**

- thaki-ui와 tds 모두 Input과 Textarea가 분리되어 있음
- tds Input은 `variant`, `label`, `helperText` 등 추가 기능 제공
- tds Input size는 `sm`/`md` 기본 + `xs`/`lg` 호환 alias 지원
...
| ------------------------ | ------------------------------------- | --------------------------------------------------------------------------- | ------------------------- |
| **컴포넌트 구조**        | 단일 컴포넌트 (children에 props 주입) | Simple API + Compound API 모두 지원                                         | ✅ Simple API로 호환 가능 |
| **label**                | `string` (필수)                       | `string?` (Simple API) 또는 `FormField.Label` (Compound)                    | ✅ 호환                   |
| **required**             | `boolean`                             | `boolean?`                                                                  | ✅ 호환                   |
| **hint**                 | `string?`                             | `helperText?: string` (Simple API) 또는 `FormField.HelperText` (Compound)   | ✅ 호환 (이름 변경)       |
| **error**                | `string?`                             | `boolean?` (Simple) + `errorMessage?: string`                               | ✅ 호환 (분리됨)          |
| **success**              | `string?`                             | 없음                                                                        | ⚠️ 제거됨                 |
| **description**          | `ReactNode?`                          | `description?: string` (Simple API) 또는 `FormField.Description` (Compound) | ✅ 호환                   |
| **disabled**             | `boolean?`                            | `boolean?`                                                                  | ✅ 호환                   |
| **children**             | `ReactElement` (단일 컨트롤)          | `ReactNode` (Simple: 단일 컨트롤 / Compound: Label+Control 등)              | ✅ 호환                   |
| **id**                   | 없음                                  | `string?`                                                                   | ✅ tds에 추가됨           |
| **spacing**              | 없음                                  | `'default' \| 'loose'`                                                      | ✅ tds에 추가됨           |
| **FormField.Label.size** | 없음                                  | `'sm' \| 'md'`                                                              | ✅ tds에 추가됨           |

**주요 차이점:**

- tds FormField는 Simple API(`label`, `helperText`, `description` props)와 Compound API(`FormField.Label` 등) 모두 지원
- thaki-ui의 Simple 패턴은 tds Simple API로 대부분 호환 가능
- thaki-ui의 `success` 기능이 tds에서 제거됨
- tds는 `spacing` prop 추가 (Checkbox, Toggle용 `loose` 간격)
...
### 구조가 다른 컴포넌트 (API 통일 논의 필요)

| 컴포넌트              | thaki-ui                 | TDS                        | 차이      |
| --------------------- | ------------------------ | -------------------------- | --------- |
| **Dropdown → Select** | Compound (children)      | 배열 props                 | 구조 변경 |
| **RadioGroup**        | options 배열 자동 렌더링 | 개별 Radio + Context       | 구조 변경 |
| **Toast**             | 단일 컴포넌트            | Provider 패턴              | 구조 변경 |
| **Disclosure**        | 단일 컴포넌트            | Compound (Trigger + Panel) | 구조 변경 |
| **ContextMenu**       | Compound (Root + Item)   | 배열 props                 | 구조 변경 |

### Props 이름/타입이 다른 컴포넌트

| 컴포넌트            | 변경 내용                                           |
| ------------------- | --------------------------------------------------- |
| **Pagination**      | `totalCount` + `size` → `totalPages` + `totalItems` |
| **ProgressBar**     | `variant` (색상) → `status` (상태)                  |
| **StatusIndicator** | `colorScheme`, `customIcon` 제거, 상태 타입 확장    |
| **DatePicker**      | `onApply`/`onCancel` → 즉시 `onChange`              |
| **Breadcrumb**      | `path` → `href`                                     |
| **TabBar**          | `onTabClick` → `onTabChange`, `title` → `label`     |

### 호환 가능한 컴포넌트

| 컴포넌트              | 비고                                                                |
| --------------------- | ------------------------------------------------------------------- |
| **Input / Textarea**  | size alias(`xs`/`lg`) 지원, `variant`/`fullWidth` 추가됨            |
| **FormField**         | Simple API로 호환 가능 (`label`, `helperText`, `description` props) |
| **Toggle**            | `label`, `description` 추가됨                                       |
| **Tooltip**           | `delay`, `disabled` 추가됨                                          |
| **InlineMessage**     | `type` → `variant`, `closable` 제거됨                               |
| **MonitoringToolbar** | API 거의 동일, 기본값만 다름                                        |
| **FloatingCard**      | 접기/펼치기 API 다름, 액션 버튼 추가됨                              |
