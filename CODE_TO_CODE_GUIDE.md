...
| -------------- | ---------------------------- | -------------------------------- | --------------------------------------------------------------------------- |
| `PageShell`    | 전체 페이지 레이아웃         | `AppLayout`                      | config 객체 기반. `sidebar` → `SidebarComponent`, `topBar` → `ToolBar` 내장 |
| `PageHeader`   | 페이지 제목 + 액션 버튼      | `Title` + 커스텀 조합            | 직접 대응 없음. `Title size="large"` + HStack으로 구성                      |
...
| TDS 컴포넌트           | 역할                      | ThakiUI 컴포넌트                      | 비고                                                                                      |
| ---------------------- | ------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------- |
| `Button`               | 버튼                      | `Button`                              | `variant`: danger→error. `appearance` 별도 prop (solid/outline/ghost). size에 `'xs'` 추가 |
...
| `FilterSearchInput`    | 고급 필터 검색            | `FilterSearchInput`                   | `filters` → `filterKeys: FilterKey[]`. API 구조 다름 (섹션 6.1 참조)                      |
...
| TDS 컴포넌트      | 역할                      | ThakiUI 컴포넌트   | 비고                                                                                              |
| ----------------- | ------------------------- | ------------------ | ------------------------------------------------------------------------------------------------- |
| `Table`           | 데이터 테이블             | `TcTable`          | **가장 큰 API 차이** — render-prop 기반 (섹션 6.2 참조)                                           |
| `Pagination`      | 페이지 네비게이션         | `Pagination`       | `currentPage`→`currentAt`, `pageSize`→`size`, `totalItems`→`totalCount`                           |
| `Badge`           | 뱃지/태그                 | `Badge`            | `variant` → `theme` (약어: `'blu'`/`'red'`/`'gre'`/`'ylw'`/`'gry'`), `type`: `'subtle'`/`'solid'` |
| `BadgeList`       | 뱃지 목록 (overflow)      | `MultiItemDisplay` | API 확인 필요                                                                                     |
| `Chip`            | 필터 칩                   | `Tag`              | 이름 변경. 기능 유사                                                                              |
| `StatusIndicator` | 상태 표시                 | `StatusIndicator`  | `status` → `variant`. `colorScheme` 별도 prop 추가                                                |
...
| TDS 컴포넌트               | 역할                 | ThakiUI 컴포넌트 | 비고                                                                                 |
| -------------------------- | -------------------- | ---------------- | ------------------------------------------------------------------------------------ |
| `Tabs` / `TabList` / `Tab` | 탭 네비게이션        | `Tabs` + `Tab`   | `value`→`activeTabId`, `variant`: underline→line, boxed→button                       |
| `TabBar`                   | 브라우저 스타일 탭바 | `TabBar`         | 거의 동일                                                                            |
| `TopBar`                   | 상단바               | `ToolBar`        | `breadcrumb` → `breadcrumbItems`, `actions` → `rightActions`. `navigation` 객체 필요 |
| `Breadcrumb`               | 경로 표시            | `Breadcrumb`     | `items` 구조 유사. `path` → `path`, `label` → `label`                                |

### Overlay

| TDS 컴포넌트   | 역할                   | ThakiUI 컴포넌트                            | 비고                                                                   |
| -------------- | ---------------------- | ------------------------------------------- | ---------------------------------------------------------------------- |
| `Modal`        | 모달 다이얼로그        | `Overlay.Template type="modal"`             | `isOpen`→`appeared`, `onClose`→`onCancel`, `size`: sm/md               |
| `ConfirmModal` | 확인/삭제 모달         | `ActionModal` 또는 `DeleteResourceModal`    | `confirmVariant` → `actionButtonVariant` (error/primary/secondary)     |
| `Drawer`       | 사이드 패널 (폼)       | `Overlay.Template type="drawer-horizontal"` | `isOpen`→`appeared`, `footer`→`confirmUI`+`cancelUI`. 통합 Overlay API |
...
| TDS 컴포넌트    | 역할           | ThakiUI 컴포넌트                 | 비고                                                             |
| --------------- | -------------- | -------------------------------- | ---------------------------------------------------------------- |
| `InlineMessage` | 인라인 알림    | `InlineMessage`                  | `variant` → `type` (success/info/warning/error)                  |
...
| `SectionCard.DataRow`   | 라벨-값 행     | `DetailCardField`           | `label`+`value` 동일. `isLink` → `type: 'component'`+커스텀 |
...
| `FormField` (Open Section) | 위자드 폼 필드  | `Fieldset` + `FormField` | `Fieldset variant="bordered"` + `legend`으로 섹션 구분      |
...
- `filters` → `filterKeys`
- `appliedFilters`+`onFiltersChange` → `onFilterAdd` 콜백 + `useFilterSearch` 훅

---

### 6.2 Table → TcTable 변환 가이드

**가장 큰 API 차이**입니다. TDS `Table`은 `columns`+`data` props 기반이고, ThakiUI `TcTable`은 render-prop 기반입니다.
...
const columnNames = [
  ...
  { key: 'createdAt', header: 'Created', align: 'right', sortable: true },
];
...
- TDS: `columns[].render` 함수에서 셀 커스텀 렌더링
- ThakiUI: `TcTable.Body` children 함수(render-prop)에서 행 전체를 렌더링
- TDS: `selectable` + `selectedKeys` boolean/배열 방식
- ThakiUI: `type="checkbox"` + `selectedValues` + `getSelectValue` 함수 방식
- TDS: `data` prop
- ThakiUI: `rowData` prop
...
| **InlineMessage**   | `variant`             | `type`                      | 이름 변경                                      |
...
| ThakiUI 컴포넌트      | 역할                                   | 활용 시점                        |
| --------------------- | -------------------------------------- | -------------------------------- |
| `CreateLayout`        | Create 페이지 레이아웃 (사이드바 포함) | TDS의 Create 페이지 변환 시      |
| `FloatingCard`        | 요약/Quota 사이드바                    | Create 위자드의 사이드바 패널    |
| `Fieldset`            | 폼 섹션 그룹핑                         | TDS의 Open Section Card 대체     |
| `ResourceActionModal` | 리소스 액션 모달 (InfoContainer 포함)  | ConfirmModal 변환 시             |
| `DeleteResourceModal` | 삭제 확인 모달 (타겟 목록)             | 벌크 삭제 모달                   |
| `TabSelector`         | 세그먼트 스타일 탭                     | 작은 옵션 그룹 전환              |
| `ExpandableTable`     | 확장 가능 테이블                       | Detail 페이지 내 중첩 데이터     |
| `SelectableTable`     | 선택 가능 테이블 (Table 확장)          | Table + 선택 기능 필요 시        |
| `TableSettingDrawer`  | 테이블 컬럼 설정                       | TDS의 ViewPreferencesDrawer 대체 |
...
// ThakiUI 변환
import { TcTable, Button, Overlay, Title, StatusIndicator } from '@thaki/shared';
```
...
| 컴포넌트                       | 역할                    | 변환 방안               |
| ------------------------------ | ----------------------- | ----------------------- |
| `Sidebar`                      | 사이드바 네비게이션     | 프론트엔드 팀 자체 구현 |
| `ContainerSidebar`             | Container 전용 사이드바 | 프론트엔드 팀 자체 구현 |
