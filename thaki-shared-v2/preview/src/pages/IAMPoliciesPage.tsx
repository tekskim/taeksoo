...
import { IconDownload, IconTrash, IconChevronDown, IconChevronRight, IconX } from '@tabler/icons-react';
...
const mockPolicies: Policy[] = [
  { id: 'p-001', name: 'FullAccess', type: 'Built-in', apps: 'All (*)', roles: 'admin', description: 'Full access to all resources', editedAt: 'Sep 12, 2025', permissions: [{ application: 'All', partition: '*', resource: '*', actions: ['Read', 'List', 'Write', 'Delete', 'Admin'] }] },
  { id: 'p-002', name: 'ReadCompute', type: 'Built-in', apps: 'compute:tenantA (+3)', roles: 'compute-admin (+1)', description: '-', editedAt: 'Sep 12, 2025', permissions: [{ application: 'Compute', partition: 'tenantA', resource: 'Instance', actions: ['Read', 'List'] }, { application: 'Compute', partition: 'tenantA', resource: 'Volume', actions: ['Read', 'List', 'Write'] }] },
  { id: 'p-003', name: 'ViewNetwork', type: 'Built-in', apps: 'network (+1)', roles: 'network-viewer', description: 'Read-only network access', editedAt: 'Aug 20, 2025' },
  { id: 'p-004', name: 'ManageStorage', type: 'Custom', apps: 'storage (+2)', roles: 'storage-manager', description: 'Storage management policy', editedAt: 'Aug 15, 2025', permissions: [{ application: 'Storage', partition: '-', resource: 'Volume', actions: ['Read', 'List', 'Write', 'Delete'] }, { application: 'Storage', partition: '-', resource: 'Snapshot', actions: ['Read', 'List', 'Write'] }] },
  { id: 'p-005', name: 'DeveloperAccess', type: 'Custom', apps: 'compute (+2)', roles: 'developer', description: 'Developer access policy', editedAt: 'Sep 1, 2025' },
  { id: 'p-006', name: 'QAAccess', type: 'Custom', apps: 'compute (+1)', roles: 'qa-tester', description: 'QA tester access', editedAt: 'Sep 5, 2025' },
  { id: 'p-007', name: 'SecurityFullAccess', type: 'Built-in', apps: 'iam (+2)', roles: 'security-admin', description: 'Security administration', editedAt: 'Jul 10, 2025' },
  { id: 'p-008', name: 'ViewBilling', type: 'Built-in', apps: 'billing', roles: 'billing-viewer', description: 'View billing information', editedAt: 'Jul 1, 2025' },
  { id: 'p-009', name: 'SupportAccess', type: 'Custom', apps: 'support (+2)', roles: 'support-agent', description: 'Customer support access', editedAt: 'Aug 10, 2025' },
  { id: 'p-010', name: 'ReadAll', type: 'Built-in', apps: 'All (*)', roles: 'read-only', description: 'Read-only access to all', editedAt: 'Jun 20, 2025' },
];
    ...
    <div className="border-t border-border p-4 bg-surface">
      <div className="flex flex-col gap-1">
        <div className="flex items-stretch min-h-9 bg-surface-muted border border-border rounded-md">
          <div className="w-10 px-3 py-1.5 text-11 font-medium leading-16 text-text flex items-center">#</div>
          <div className="flex-1 px-3 py-1.5 text-11 font-medium leading-16 text-text flex items-center border-l border-border">Application</div>
          <div className="flex-1 px-3 py-1.5 text-11 font-medium leading-16 text-text flex items-center border-l border-border">Partition</div>
          <div className="flex-1 px-3 py-1.5 text-11 font-medium leading-16 text-text flex items-center border-l border-border">Resource</div>
          <div className="flex-[2] px-3 py-1.5 text-11 font-medium leading-16 text-text flex items-center border-l border-border">Action</div>
        </div>
          ...
          <div key={index} className="flex items-stretch min-h-9 bg-surface border border-border rounded-md">
            ...
            <div className="flex-[2] min-w-0 px-3 py-1.5 text-12 leading-16 text-text flex items-center gap-1 flex-wrap">
              {perm.actions.map((action, i) => (
                <Badge key={i} theme="gry" size="sm">{action}</Badge>
              ))}
            </div>
          </div>
        ))}
...
const filterKeys: FilterKey[] = [
  ...
  { key: 'type', label: 'Type', type: 'select', options: [{ value: 'Built-in', label: 'Built-in' }, { value: 'Custom', label: 'Custom' }] },
...
export function IAMPoliciesPage() {
  ...
  const allSelected = paginatedPolicies.length > 0 && selectedRows.length === paginatedPolicies.length;
