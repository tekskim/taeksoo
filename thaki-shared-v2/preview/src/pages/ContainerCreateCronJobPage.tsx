...
type CronJobSummaryStep = 'basic-info' | 'labels-annotations' | 'scaling';

const CRONJOB_SUMMARY_ORDER: CronJobSummaryStep[] = ['basic-info', 'labels-annotations', 'scaling'];

const CRONJOB_SUMMARY_LABELS: Record<CronJobSummaryStep, string> = {
  'basic-info': 'Basic information',
  'labels-annotations': 'Labels & Annotations',
  scaling: 'Scaling and Upgrade Policy',
};
...
const POD_SUMMARY_ORDER: PodSummaryStep[] = [
  'basic-info',
  'labels-annotations',
  'scaling',
  'networking',
...
const POD_SUMMARY_LABELS: Record<PodSummaryStep, string> = {
  'basic-info': 'Basic Information',
  'labels-annotations': 'Labels & Annotations',
  scaling: 'Scaling and Upgrade Policy',
...
const CONTAINER_SUMMARY_ORDER: ContainerSummaryStep[] = [
  'basic-info',
  'image',
  'command',
  'environment-variables',
...
const CONTAINER_SUMMARY_LABELS: Record<ContainerSummaryStep, string> = {
  'basic-info': 'Basic Information',
  image: 'Image',
  command: 'Command',
...
interface ScalingPolicySectionProps {
  completions: number;
  onCompletionsChange: (value: number) => void;
  parallelism: number;
  ...
  suspend: boolean;
  onSuspendChange: (value: boolean) => void;
}
...
function ScalingPolicySection({
  ...
  onParallelismChange,
  backOffLimit,
  onBackOffLimitChange,
  activeDeadline,
  onActiveDeadlineChange,
  successfulJobHistoryLimit,
  onSuccessfulJobHistoryLimitChange,
  failedJobHistoryLimit,
  onFailedJobHistoryLimitChange,
  startingDeadlineSeconds,
  onStartingDeadlineSecondsChange,
  concurrencyPolicy,
    ...
    <SectionCard className="pb-4">
      ...
      <SectionCard.Content showDividers={false}>
        <div className="flex flex-col gap-6">
          ...
          <FormField label="Back Off Limit">
            <div className="flex flex-row gap-2 items-center">
              <NumberInput
                value={backOffLimit}
          ...
          <FormField label="Active Deadline">
            <div className="flex flex-row gap-2 items-center">
              <NumberInput
          ...
          <FormField label="Successful Job History Limit">
            <NumberInput
          ...
          <FormField label="Failed Job History Limit">
            <NumberInput
          ...
          <FormField label="Starting Deadline Seconds">
            <div className="flex flex-row gap-2 items-center">
              <NumberInput
          ...
          <FormField label="Concurrency">
            <div className="mt-3 flex flex-col gap-2 items-start">
              <RadioGroup value={concurrencyPolicy} onChange={onConcurrencyPolicyChange}>
                <div className="flex flex-col gap-2">
                  <Radio value="allow" label="Allow CronJobs to run concurrently" />
                  <Radio value="forbid" label="Skip next run if current run hasn't finished" />
                  <Radio value="replace" label="Replace run if current run hasn't finished" />
                </div>
              </RadioGroup>
            </div>
          </FormField>

          <FormField label="Suspend">
            <div className="mt-3 flex flex-col gap-2 items-start">
              <RadioGroup
                value={suspend ? 'yes' : 'no'}
                onChange={(v) => onSuspendChange(v === 'yes')}
              >
                <div className="flex flex-col gap-2">
...
export function ContainerCreateCronJobPage() {
  ...
  // Scaling & Upgrade Policy state (CronJob-specific)
  ...
  const floatingSummaryItems = useMemo(() => {
    if (activeTab === 'cronjob') {
      return CRONJOB_SUMMARY_ORDER.map((key) => {
        let state: WizardSectionState = 'pre';
        if (key === 'basic-info') state = name.trim() ? 'done' : 'active';
        else state = 'done';
        return { label: CRONJOB_SUMMARY_LABELS[key], state };
      });
    }
    if (activeTab === 'pod') {
      return POD_SUMMARY_ORDER.map((key) => ({
        label: POD_SUMMARY_LABELS[key],
        state: 'done' as WizardSectionState,
      }));
    }
    return CONTAINER_SUMMARY_ORDER.map((key) => ({
      label: CONTAINER_SUMMARY_LABELS[key],
      state: 'done' as WizardSectionState,
    }));
  }, [activeTab, name]);
  ...
  return (
    <CreateLayout
      header={
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h1 className="text-heading-h4 text-text">Create CronJob</h1>
            <p className="text-body-md text-[var(--color-text-subtle)]">
              Create a CronJob to schedule Jobs to run periodically at specified times or intervals.
            </p>
          </div>
          <div className="w-full border-b border-border">
            <div ref={tabListRef} className="flex items-end [&_[role=tablist]]:border-b-0">
              <Tabs
                      ...
                      tab.closable ? (
                        <span className="inline-flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="size-4 flex items-center justify-center rounded-sm hover:bg-surface-hover shrink-0 text-text-subtle hover:text-text transition-colors"
                          >
              ...
              <button
                ...
                className="shrink-0 flex items-center justify-center size-7 rounded-sm text-text-subtle transition-colors hover:bg-surface-hover hover:text-text mb-1.5"
              >
      ...
      sidebar={
        <FloatingCard
          ...
          sections={[
            {
              items: floatingSummaryItems.map(({ label, state }) => ({
                label,
        ...
        {activeTab === 'cronjob' && (
          <>
            ...
            <ScalingPolicySection
              ...
              onBackOffLimitChange={setBackOffLimit}
              activeDeadline={activeDeadline}
              onActiveDeadlineChange={setActiveDeadline}
              successfulJobHistoryLimit={successfulJobHistoryLimit}
              onSuccessfulJobHistoryLimitChange={setSuccessfulJobHistoryLimit}
              failedJobHistoryLimit={failedJobHistoryLimit}
              onFailedJobHistoryLimitChange={setFailedJobHistoryLimit}
              startingDeadlineSeconds={startingDeadlineSeconds}
              onStartingDeadlineSecondsChange={setStartingDeadlineSeconds}
              concurrencyPolicy={concurrencyPolicy}
              onConcurrencyPolicyChange={setConcurrencyPolicy}
              suspend={suspend}
              onSuspendChange={setSuspend}
            />
          </>
        )}
        ...
        {activeTab === 'pod' && (
          <>
            ...
            <SectionCard className="pb-4">
              ...
              <SectionCard.Content showDividers={false}>
                <div className="flex flex-col gap-6">
                  <h6 className="text-heading-h6 text-[var(--color-text-default)]">Pod Policy</h6>
                        ...
                        {resolverOptions.map((opt, index) => (
                          ...
                          >
                            <Input
                              ...
                              value={opt.name}
                              onChange={(e) => updateResolverOption(index, 'name', e.target.value)}
                              className="w-full"
                            />
                            <Input
                              placeholder="input value"
                              value={opt.value}
                              onChange={(e) => updateResolverOption(index, 'value', e.target.value)}
                              className="w-full"
                            />
                            <button
                        ...
                        {hostAliases.map((alias, index) => (
                          ...
                          >
                            ...
                            <Input
                              ...
                              onChange={(e) => updateHostAlias(index, 'hostname', e.target.value)}
                              className="w-full"
                            />
                            <button
              ...
              <SectionCard.Content showDividers={false}>
                <div className="flex flex-col gap-4">
                  ...
                  {!isV2 && nodeScheduling === 'specific' && (
                    <div className="flex flex-col gap-1 w-full">
                      <span className="text-label-lg text-[var(--color-text-default)]">Node</span>
                      <Select
                        options={[
                          { value: 'node-1', label: 'node-1' },
                          { value: 'node-2', label: 'node-2' },
                                ...
                                return (
                                  <Table.Tr key={row.id} rowData={row}>
                                    ...
                                    <Table.Td rowData={row} column={POD_AFFINITY_NS_COLUMNS[1]}>
                                      <span
                                        className="truncate block min-w-0 font-medium text-primary"
                                        title={row.name}
                                      >
                                        {row.name}
                                      </span>
                              ...
                              >
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    newTerms[termIndex] = {
                                      ...newTerms[termIndex],
                                      matchExpressions: newTerms[termIndex].matchExpressions.filter(
                                        (_, i) => i !== exprIndex
                                      ),
                                    };
                                    setPodAffinityTerms(newTerms);
                          ...
                          <>
                            <div className="flex flex-col gap-6 w-full">
                              ...
                              <div className="flex flex-col gap-2 w-full">
                                ...
                                <Select
                                  ...
                                  onChange={(val) => updateVolume(index, { configMapName: val })}
                            ...
                            <div className="flex flex-row gap-2 items-center">
                              <Checkbox
                                ...
                                onChange={(checked) => updateVolume(index, { optional: checked })}
                              />
                              <span className="text-label-lg text-[var(--color-text-default)]">
                                Optional
                              </span>
                        ...
                        {volume.type === 'secret' && (
                          <>
                            <div className="flex flex-col gap-6 w-full">
                              ...
                              <div className="flex flex-col gap-2 w-full">
                                <span className="text-label-lg text-[var(--color-text-default)]">
                                  Secret <span className="text-[var(--color-state-danger)]">*</span>
                                </span>
                                <Select
                                  options={[
                          ...
                          <>
                            ...
                            <div className="flex flex-row gap-2 items-center">
                              <Checkbox
                                ...
                                onChange={(checked) => updateVolume(index, { readOnly: checked })}
                              />
                              <span className="text-label-lg text-[var(--color-text-default)]">
                                Read Only
                              </span>
                            ...
                            <div className="w-full">
                              <div className="flex flex-col gap-6">
                                <div className="flex flex-col gap-3">
                                  ...
                                  <RadioGroup
                                    value={
                                      (volume as CreatePVCVolume).useExistingPV ? 'existing' : 'new'
                                    }
                                    onChange={(val) =>
                                      updateVolume(index, {
                        ...
                        <div className="flex gap-6 w-full">
                          <div className="flex flex-col gap-2 flex-1">
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="text-body-md text-[var(--color-text-subtle)]">
                                Specify the minimum number of consecutive failed checks to consider
                                the status unhealthy.
                              </span>
                            </div>
                            <NumberInput
                              ...
                              (header: { name: string; value: string }, index: number) => (
                                ...
                                >
                                  <Input
                                    ...
                                    value={header.name}
                                    onChange={(e) => {
                                      const newHeaders = [...(probe?.httpGet?.httpHeaders || [])];
                                      newHeaders[index] = {
                                        ...newHeaders[index],
                                        name: e.target.value,
                                      };
                                  ...
                                  <button
                                    className="size-5 flex items-center justify-center hover:bg-[var(--color-surface-muted)] rounded transition-colors"
                                    onClick={() => {
                                      const newHeaders = (probe?.httpGet?.httpHeaders || []).filter(
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="bg-[var(--color-surface-subtle)] rounded-[6px] px-4 py-3 w-full">
                                <div className="flex flex-col gap-1.5">
                                  ...
                                  {(config.lifecycleHooks?.preStop?.httpGet?.httpHeaders || []).map(
                                    (header, index) => (
                                      <div
                                        key={index}
                                        className="grid grid-cols-[1fr_1fr_auto] gap-1 w-full items-center"
                            ...
                            <div className="flex flex-col gap-6">
                              ...
                              <div className="flex gap-6 w-full">
                                ...
                                <div className="flex flex-col gap-2 flex-1">
                                  ...
                                  <div className="flex flex-row gap-2 items-center">
                                    <NumberInput
                                      value={
                                        parseInt(config.readinessProbe?.periodSeconds || '10') || 10
                                      }
                                      onChange={(val) =>
                                        updateProbe('readinessProbe', {
                                ...
                                {config.readinessProbe?.type !== 'httpGet' && (
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex flex-row gap-2 items-center">
                                      <NumberInput
                                        value={
                                          parseInt(config.readinessProbe?.timeoutSeconds || '1') ||
                                          1
                                        }
                                        onChange={(val) =>
                                          updateProbe('readinessProbe', {
                              ...
                              <div className="flex gap-6 w-full">
                                ...
                                <div className="flex flex-col gap-2 flex-1">
                                  ...
                                  <NumberInput
                                    value={
                                      parseInt(config.readinessProbe?.successThreshold || '1') || 1
                                    }
                                    onChange={(val) =>
                                      updateProbe('readinessProbe', {
                                ...
                                {config.readinessProbe?.type !== 'httpGet' && (
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <NumberInput
                                      value={
                                        parseInt(config.readinessProbe?.failureThreshold || '3') ||
                                        3
                                      }
                                      onChange={(val) =>
                                        updateProbe('readinessProbe', {
                              ...
                              {config.readinessProbe?.type === 'httpGet' && (
                                <div className="flex flex-col gap-2">
                                  ...
                                  <NumberInput
                                    value={
                                      parseInt(config.readinessProbe?.failureThreshold || '3') || 3
                                    }
                                    onChange={(val) =>
                                      updateProbe('readinessProbe', {
                                        failureThreshold: String(val),
                              ...
                              {config.readinessProbe?.type === 'httpGet' && (
                                <div className="flex flex-col gap-2">
                                  ...
                                  <div className="bg-[var(--color-surface-subtle)] rounded-[6px] px-4 py-3 w-full">
                                    <div className="flex flex-col gap-1.5">
                                      {(config.readinessProbe?.httpGet?.httpHeaders || []).length >
                                        0 && (
                                        <div className="grid grid-cols-[1fr_1fr_auto] gap-1 w-full items-center">
                                          <label className="text-label-sm text-[var(--color-text-default)]">
                                            Name
                                ...
                                <div className="flex gap-6 w-full">
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex flex-row gap-2 items-center">
                                      <NumberInput
                                        value={
                                          parseInt(config.livenessProbe?.periodSeconds || '10') ||
                                          10
                                        }
                                        onChange={(val) =>
                                          updateProbe('livenessProbe', {
                                ...
                                <div className="flex gap-6 w-full">
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex flex-row gap-2 items-center">
                                      <NumberInput
                                        value={
                                          parseInt(config.livenessProbe?.timeoutSeconds || '1') || 1
                                        }
                                        onChange={(val) =>
                                          updateProbe('livenessProbe', {
                                            timeoutSeconds: String(val),
                                ...
                                <div className="flex gap-6 w-full">
                                  <div className="flex flex-col gap-2 flex-1">
                                    <div className="flex flex-col gap-1">
                                      ...
                                      <span className="text-body-md text-[var(--color-text-subtle)]">
                                        Specify the minimum number of consecutive successful checks
                                        to consider the status healthy.
                                      </span>
                                    </div>
                                    <NumberInput
                                      value={
                                        parseInt(config.livenessProbe?.successThreshold || '1') || 1
                                      }
                                      onChange={(val) =>
                                        updateProbe('livenessProbe', {
                                          successThreshold: String(val),
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <NumberInput
                                      value={
                                        parseInt(config.livenessProbe?.failureThreshold || '3') || 3
                                      }
                                      onChange={(val) =>
                                        updateProbe('livenessProbe', {
                                          failureThreshold: String(val),
                              ...
                              <div className="flex flex-col gap-6">
                                ...
                                <div className="flex gap-6 w-full">
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex flex-row gap-2 items-center">
                                      <NumberInput
                                        value={
                                          parseInt(config.startupProbe?.periodSeconds || '10') || 10
                                        }
                                        onChange={(val) =>
                                          updateProbe('startupProbe', {
                                            periodSeconds: String(val),
                                ...
                                <div className="flex gap-6 w-full">
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <div className="flex flex-row gap-2 items-center">
                                      <NumberInput
                                        value={
                                          parseInt(config.startupProbe?.timeoutSeconds || '1') || 1
                                        }
                                        onChange={(val) =>
                                          updateProbe('startupProbe', {
                                            timeoutSeconds: String(val),
                                ...
                                <div className="flex gap-6 w-full">
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <NumberInput
                                      value={
                                        parseInt(config.startupProbe?.successThreshold || '1') || 1
                                      }
                                      onChange={(val) =>
                                        updateProbe('startupProbe', {
                                          successThreshold: String(val),
                                  ...
                                  <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <NumberInput
                                      value={
                                        parseInt(config.startupProbe?.failureThreshold || '3') || 3
                                      }
                                      onChange={(val) =>
                                        updateProbe('startupProbe', {
                                          failureThreshold: String(val),
                    ...
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="flex gap-6 w-full">
                        ...
                        <div className="flex flex-col gap-2 flex-1">
                          ...
                          <NumberInput
                            value={config.memoryLimit ? parseInt(config.memoryLimit) : undefined}
                            onChange={(val) =>
                              updateContainerConfig(containerId, {
