...
export function ComputeAdminVolumeBackupDetailPage() {
  ...
  const tenantHeaderValue =
    backup.tenant === '-' ? (
      '-'
    ) : (
      <Link to={`/compute-admin/tenants/${backup.tenant}`} className={tenantLinkClass}>
        {backup.tenant}
      </Link>
    );
  ...
  const infoFields: DetailPageHeaderInfoField[] = [
    ...
    { label: 'Tenant', value: tenantHeaderValue },
                ...
                <SectionCard.Content>
                  <SectionCard.DataRow label="Tenant">
                    {backup.tenant === '-' ? (
                      ...
                      <Link
                        to={`/compute-admin/tenants/${backup.tenant}`}
