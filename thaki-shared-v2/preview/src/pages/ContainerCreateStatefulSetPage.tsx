...
/* Summary sidebar section labels (same order as former SummarySidebar) */
const STATEFULSET_SUMMARY_ITEMS = [
  'Basic information',
  'Labels & Annotations',
  'Scaling and Upgrade Policy',
] as const;

const POD_SUMMARY_SECTIONS = [
  'Basic Information',
  'Labels & Annotations',
  'Scaling and Upgrade Policy',
  'Networking',
...
const CONTAINER_SUMMARY_SECTIONS = [
  'Basic Information',
  'Image',
  'Command',
...
export function ContainerCreateStatefulSetPage() {
  ...
  const handleCancel = useCallback(() => {
    navigate('/container/stateful-sets');
  }, [navigate]);
    ...
    () => [
      {
        id: 'deployment',
        title: 'Deployment',
        status: mapStatus(name.trim() ? 'done' : activeTab === 'statefulset' ? 'active' : 'pre'),
        items: [
          {
            label: STATEFULSET_SUMMARY_ITEMS[0],
            status: mapStatus(
              name.trim() ? 'done' : activeTab === 'statefulset' ? 'active' : 'pre'
            ),
          },
          ...STATEFULSET_SUMMARY_ITEMS.slice(1).map((label) => ({
            label,
      ...
      },
      {
        ...
        title: 'Pod',
        status: mapStatus('done'),
        items: POD_SUMMARY_SECTIONS.map((label) => ({
          label,
          status: mapStatus('done'),
        })),
      },
      ...containerTabs.map((ct) => ({
        id: ct.id,
        title: ct.name,
        status: mapStatus(containerTabs.length > 0 ? 'done' : 'pre'),
        items: CONTAINER_SUMMARY_SECTIONS.map((label) => ({
          label,
          status: mapStatus('done'),
        })),
      })),
    ],
    [name, activeTab, containerTabs]
  );
  ...
  return (
    <CreateLayout
      header={
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h1 className="text-heading-h4 text-text">Create StatefulSet</h1>
            <p className="text-body-md text-[var(--color-text-subtle)]">
