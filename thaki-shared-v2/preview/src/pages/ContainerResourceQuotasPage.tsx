...
const filterKeys: FilterKey[] = [
  ...
  { key: 'request', label: 'Request', type: 'input', placeholder: 'Enter request...' },
  { key: 'limit', label: 'Limit', type: 'input', placeholder: 'Enter limit...' },
...
export function ContainerResourceQuotasPage() {
  ...
  const filteredRows = useMemo(
    () =>
      resourceQuotasData.filter((r) =>
        appliedFilters.every((f) => {
          const val = String(f.value ?? '').toLowerCase();
          if (!val) return true;
          return String(r[f.key as keyof ResourceQuotaRow] ?? '')
            .toLowerCase()
            .includes(val);
        })
      ),
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1">
          <FilterSearchInput
            ...
            placeholder="Search resource quotas by attributes"
