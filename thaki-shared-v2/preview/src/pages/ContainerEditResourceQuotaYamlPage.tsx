...
export function ContainerEditResourceQuotaYamlPage() {
  ...
  return (
    <div className="flex flex-col gap-6 flex-1 min-h-0">
      <div className="flex flex-col gap-2 flex-shrink-0">
        <Title title={`Edit resource quota YAML${id ? ` — ${id}` : ''}`} size="large" />
        <p className="text-body-md text-[var(--color-text-subtle)]">
          Edit the YAML configuration of this resource quota.
        </p>
      </div>
      ...
      <div className="flex-shrink-0 h-[61px] flex items-center justify-between border-t border-[var(--color-border-strong)]">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="secondary"
            size="md"
            onClick={() => navigate('/container/resource-quotas')}
          >
            Cancel
          </Button>
          <Button
