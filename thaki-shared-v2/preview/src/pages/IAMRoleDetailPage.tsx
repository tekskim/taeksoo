    ...
    <div className="border-t border-border p-4 bg-surface">
      <div className="flex flex-col gap-1">
        <div className="flex items-stretch min-h-9 bg-surface-muted border border-border rounded-md">
          ...
          <div className="flex-[2] px-3 py-2 text-11 font-medium leading-16 text-text flex items-center border-l border-border">
            Action
          </div>
        </div>
...
export function IAMRoleDetailPage() {
  ...
  const entitiesFilterKeys: FilterKey[] = [
    { key: 'name', label: 'Name', type: 'input', placeholder: 'Enter name...' },
  ];

  const filteredPolicies = applyFilter(
    mockRolePolicies as unknown as Record<string, unknown>[],
    policiesFilters
  ) as unknown as typeof mockRolePolicies;
  ...
  const filteredUserGroups = applyFilter(
    mockAttachedUserGroups as unknown as Record<string, unknown>[],
    entitiesFilters
  ) as unknown as typeof mockAttachedUserGroups;
  const filteredUsers = applyFilter(
    mockAttachedUsers as unknown as Record<string, unknown>[],
    entitiesFilters
  ) as unknown as typeof mockAttachedUsers;
          ...
          <Tab id="policies" label="Policies">
            <div className="flex flex-col gap-4 pt-4">
              ...
              <FilterSearchInput
                ...
                onFilterAdd={(f) => {
                  setPoliciesFilters((p) => [...p, f]);
                  setPoliciesCurrentPage(1);
                }}
                selectedFilters={policiesFilters}
              ...
              <div className="flex flex-col gap-1">
                {/* Table Header */}
                <div className="flex items-stretch min-h-9 bg-surface-muted border border-border rounded-md">
                  <div className="flex-1 flex items-center px-3 py-1.5 text-11 font-medium leading-16 text-text">
                    Status
                  </div>
                  <div className="flex-1 flex items-center px-3 py-1.5 text-11 font-medium leading-16 text-text border-l border-border">
                    Type
                  </div>
                  <div className="flex-1 flex items-center px-3 py-1.5 text-11 font-medium leading-16 text-text border-l border-border">
                    Apps
                  ...
                  <div className="w-[72px] flex items-center justify-center px-3 py-1.5 text-11 font-medium leading-16 text-text border-l border-border">
                    Action
                  </div>
                </div>
                {paginatedPolicies.map((policy) => (
                  ...
                  >
                    <div className="flex items-center min-h-9 hover:bg-surface-muted transition-colors">
                      ...
                      <div className="flex-1 min-w-0 flex items-center px-3 py-1.5 text-12 leading-16 text-text truncate">
                        {policy.type}
                      </div>
                      <div className="flex-1 min-w-0 flex items-center px-3 py-1.5 text-12 leading-16 text-text truncate">
                        {policy.apps}
                      ...
                      <div className="flex-1 min-w-0 flex items-center px-3 py-1.5 text-12 leading-16 text-text whitespace-nowrap">
                        {policy.editedAt}
                      </div>
                      <div className="w-[72px] flex items-center justify-center px-3 py-2 border-l border-transparent">
                        ...
                        >
                          <ContextMenu.Item
                            action={() => console.log('Detach policy', policy.id)}
                  ...
                  <Table columns={userGroupColumns} rows={paginatedUserGroups}>
                    {paginatedUserGroups.map((row) => (
                      <Table.Tr key={row.id} rowData={row}>
                        ...
                        <Table.Td rowData={row} column={userGroupColumns[1]}>
                          {row.type}
                        </Table.Td>
                        <Table.Td rowData={row} column={userGroupColumns[2]}>
                          {row.userCount}
                        </Table.Td>
                        <Table.Td rowData={row} column={userGroupColumns[3]}>
