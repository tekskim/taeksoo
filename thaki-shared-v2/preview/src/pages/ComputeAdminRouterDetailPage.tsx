...
const tenantLinkClass =
  'text-12 leading-18 font-medium text-primary hover:underline no-underline truncate';

export function ComputeAdminRouterDetailPage() {
    ...
    { label: 'ID', value: router.id, showCopyButton: true, copyText: router.id },
    {
      ...
      value:
        router.tenant === '-' ? (
          ...
          <Link to={`/compute-admin/tenants/${router.tenant}`} className={tenantLinkClass}>
            {router.tenant}
          </Link>
        ),
    },
                ...
                <SectionCard.Content>
                  <SectionCard.DataRow label="Tenant">
                    {router.tenant === '-' ? (
                      ...
                      <Link
                        to={`/compute-admin/tenants/${router.tenant}`}
