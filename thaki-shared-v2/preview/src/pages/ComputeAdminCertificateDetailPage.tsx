...
interface Listener extends Record<string, unknown> {
  id: string;
  name: string;
...
export function ComputeAdminCertificateDetailPage() {
  ...
  const infoFields: DetailPageHeaderInfoField[] = [
    ...
    { label: 'Tenant', value: certificate.tenant },
                  ...
                  <Table.Tr key={row.id} rowData={row}>
                    ...
                    <Table.Td rowData={row} column={listenerColumns[1]}>
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <Link to={`/compute-admin/listeners/${row.id}`} className={linkClass}>
                          {row.name}
                        </Link>
                    ...
                    <Table.Td rowData={row} column={listenerColumns[4]}>
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <Link
                          to={`/compute-admin/load-balancers/${row.loadBalancer.id}`}
                          className={linkClass}
                        >
