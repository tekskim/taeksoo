...
function parseGiBToNumber(s: string): number {
  const g = s.match(/^([\d.]+)\s*GiB/i);
  if (g) return Math.max(1, Math.floor(Number(g[1])));
  const n = s.match(/^([\d.]+)/);
  return n ? Math.max(1, Math.floor(Number(n[1]))) : 10;
}

function parseRamToMiB(s: string): number {
  const m = s.match(/^([\d.]+)\s*MiB/i);
  if (m) return Math.max(1, Math.floor(Number(m[1])));
  const g = s.match(/^([\d.]+)\s*GiB/i);
  if (g) return Math.max(1, Math.floor(Number(g[1]) * 1024));
  return 2048;
}
    ...
    <div className="flex flex-col gap-6 min-w-0">
      ...
      <EditImageDrawer
        ...
        initialData={{
          ...
          minDiskGiB: parseGiBToNumber(image.minDisk),
          minRamMiB: parseRamToMiB(image.minRam),
          visibility: (image.visibility === 'Public' || image.visibility === 'Project'
            ? 'Public'
