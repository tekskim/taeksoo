...
type JobSummaryStep = 'basic-info' | 'labels-annotations' | 'scaling';

const JOB_SUMMARY_ORDER: JobSummaryStep[] = ['basic-info', 'labels-annotations', 'scaling'];

const JOB_SUMMARY_LABELS: Record<JobSummaryStep, string> = {
...
export function ContainerCreateJobPage() {
  ...
  const floatingSections = useMemo(() => {
    let rows: { label: string; state: WizardSectionState }[];
    if (activeTab === 'job') {
      rows = JOB_SUMMARY_ORDER.map((key) => {
        let state: WizardSectionState = 'pre';
        if (key === 'basic-info') state = name.trim() ? 'done' : 'active';
        else state = 'done';
        return { label: JOB_SUMMARY_LABELS[key], state };
      });
    } else if (activeTab === 'pod') {
      rows = POD_SUMMARY_ORDER.map((key) => ({
        label: POD_SUMMARY_LABELS[key],
        state: 'done' as WizardSectionState,
      }));
    } else {
      rows = CONTAINER_SUMMARY_ORDER.map((key) => ({
  ...
  return (
    <CreateLayout
      header={
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h1 className="text-heading-h4 text-text">Create job</h1>
            <p className="text-body-md text-[var(--color-text-subtle)]">
