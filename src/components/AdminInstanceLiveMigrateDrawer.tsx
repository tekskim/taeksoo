...
export interface LiveMigrateInstanceInfo {
  id: string;
  name: string;
  currentHost: string;
}

export interface AdminInstanceLiveMigrateDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  instance: LiveMigrateInstanceInfo;
  onLiveMigrate?: (hostId: string, blockMigrate: boolean) => void;
}
...
/* ----------------------------------------
   AdminInstanceLiveMigrateDrawer Component
   ---------------------------------------- */

export function AdminInstanceLiveMigrateDrawer({
  isOpen,
  onClose,
  instance,
  onLiveMigrate,
}: AdminInstanceLiveMigrateDrawerProps) {
  ...
  // Option state
  const [blockMigrate, setBlockMigrate] = useState(false);
  ...
  const handleLiveMigrate = () => {
    if (!selectedHostId) {
      setHostError(true);
      return;
    }
    setIsSubmitting(true);
    onLiveMigrate?.(selectedHostId, blockMigrate);
    setTimeout(() => {
    ...
    <Drawer
      ...
      footer={
        <HStack gap={2} justify="center" className="w-full">
          ...
          <Button variant="primary" onClick={handleLiveMigrate} className="w-[152px]">
            Live migrate
          </Button>
        </HStack>
      }
...
export default AdminInstanceLiveMigrateDrawer;
