...
function NameCell({ id, name }: NameCellProps) {
  return (
    <Link
      to={`/storage-domain-admin/images/${id}`}
...
export function ImagesPage() {
  ...
  const columns: TableColumn<Image>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex flex-col gap-1 w-full">
          <span className="text-body-sm text-[var(--color-text-default)]">{row.usage}%</span>
          <ProgressBar
            value={row.usage}
            max={100}
            size="sm"
    ...
    },
    {
      ...
      sortable: true,
      render: (_, row) => (
        <Badge theme={row.mirroring === 'Enabled' ? 'green' : 'gray'} size="sm">
          {row.mirroring}
        </Badge>
      ),
    },
