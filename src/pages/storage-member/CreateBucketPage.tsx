...
import { IconEdit, IconCirclePlus, IconX, IconInfoCircle } from '@tabler/icons-react';
            ...
            <FormField spacing="loose">
              ...
              <FormField.Control>
                <Toggle
                  ...
                  label={objectLocking === 'enabled' ? 'Enabled' : 'Disabled'}
                />
              </FormField.Control>
            </FormField>
          </div>
          ...
          {objectLocking === 'enabled' && (
            <>
                  ...
                  <FormField.Control>
                    <VStack className="gap-[var(--radio-group-item-gap)]" align="start">
                      <HStack gap={1.5} align="center">
                        <Radio
                          value="compliance"
                          checked={lockingMode === 'compliance'}
                          onChange={() => onLockingModeChange('compliance')}
                          label="Compliance"
                        />
                        <Tooltip content="In COMPLIANCE an object version cannot be overwritten or deleted for the duration of the period.">
                      ...
                      <HStack gap={1.5} align="center">
                        <Radio
                          value="governance"
                          checked={lockingMode === 'governance'}
                          onChange={() => onLockingModeChange('governance')}
                          label="Governance"
                        />
                        <Tooltip content="In GOVERNANCE mode, users cannot overwrite or delete an object version or alter its lock settings unless they have special permissions.">
                          <IconInfoCircle size={14} className="text-[var(--color-text-subtle)]" />
                        </Tooltip>
                      </HStack>
                    </VStack>
              ...
              <div className="py-6">
                <FormField
                  label="Days"
                  description="The number of days that you want to specify for the default retention period that will be applied to new objects placed in this bucket."
                  helperText="1-36500 days"
                  required
                >
                  <NumberInput
                    min={1}
                    max={36500}
                    value={retentionDays ? Number(retentionDays) : undefined}
                    onChange={(val) => onRetentionDaysChange(val !== undefined ? String(val) : '')}
                    width="sm"
                  />
                </FormField>
              </div>
...
export default function CreateBucketPage() {
  ...
  const handleCreate = useCallback(() => {
    ...
    navigate('/storage-member/buckets');
  }, [
    bucketName,
    region,
    owner,
