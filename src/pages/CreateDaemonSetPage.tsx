...
type DaemonSetSectionStep = 'basic-info' | 'labels-annotations' | 'scaling-policy';
...
const DAEMONSET_SECTION_LABELS: Record<DaemonSetSectionStep, string> = {
...
const DAEMONSET_SECTION_ORDER: DaemonSetSectionStep[] = [
  'basic-info',
  'labels-annotations',
  'scaling-policy',
];
...
interface SummarySidebarProps {
  daemonsetSectionStatus: Record<DaemonSetSectionStep, SectionState>;
...
function SummarySidebar({ 
  ...
  const [daemonsetExpanded, setDaemonsetExpanded] = useState(true);
  ...
  // Check if all daemonset sections are done
  const allDaemonSetDone = DAEMONSET_SECTION_ORDER.every(
    (key) => daemonsetSectionStatus[key] === 'done'
  );
          ...
          <VStack gap={3}>
            ...
            <VStack gap={0}>
              ...
              >
                ...
                {allDaemonSetDone && <WizardSectionStatusIcon status="done" />}
              </HStack>

              {/* DaemonSet Sub-items (Indented) */}
              {daemonsetExpanded && (
                <VStack gap={0} className="pl-4">
                  {DAEMONSET_SECTION_ORDER.map((key) => {
          ...
          {strategy === 'rolling-update' && (
            <VStack gap={4}>
              ...
              <VStack gap={2} className="max-w-[calc(50%-8px)]">
                ...
                <p className="text-[11px] text-[var(--color-text-subtle)] leading-[16px]">
                  The maximum time allowed for a DaemonSet to progress before it is considered failed.
                </p>
                <HStack gap={2}>
                  <NumberInput
...
export function CreateDaemonSetPage() {
  ...
  // Active tab (DaemonSet, Pod, Container-X)
  const [activeTab, setActiveTab] = useState('daemonset');
  ...
  // Section states for DaemonSet tab
  const [daemonsetSectionStatus, setDaemonsetSectionStatus] = useState<Record<DaemonSetSectionStep, SectionState>>({
    'basic-info': 'active',
  ...
  // Editing state
  const [editingSection, setEditingSection] = useState<DaemonSetSectionStep | null>(null);
  ...
  const handleNext = useCallback((currentSection: DaemonSetSectionStep) => {
    const currentIndex = DAEMONSET_SECTION_ORDER.indexOf(currentSection);
    const nextSection = DAEMONSET_SECTION_ORDER[currentIndex + 1];

    setDaemonsetSectionStatus((prev) => ({
      ...prev,
      [currentSection]: 'done',
      ...(nextSection && { [nextSection]: 'active' }),
    }));

    // If this was the last daemonset section, mark Pod as active
    if (!nextSection) {
      setPodStatus('done');
      setContainerStatuses((prev) => 
  ...
  // Handle edit
  const handleEdit = useCallback((section: DaemonSetSectionStep) => {
    setEditingSection(section);
    const sectionIndex = DAEMONSET_SECTION_ORDER.indexOf(section);

    setDaemonsetSectionStatus((prev) => {
      const newStatus = { ...prev };
      DAEMONSET_SECTION_ORDER.forEach((key, index) => {
  ...
  const handleEditCancel = useCallback(() => {
    ...
    setDaemonsetSectionStatus((prev) => {
      ...
      const editIndex = DAEMONSET_SECTION_ORDER.indexOf(editingSection);
      for (let i = editIndex + 1; i < DAEMONSET_SECTION_ORDER.length; i++) {
        if (newStatus[DAEMONSET_SECTION_ORDER[i]] === 'writing') {
          newStatus[DAEMONSET_SECTION_ORDER[i]] = 'done';
        }
      }
      return newStatus;
    });
  ...
  const allDaemonSetSectionsDone = DAEMONSET_SECTION_ORDER.every(
    (key) => daemonsetSectionStatus[key] === 'done'
  );
  const podDone = podStatus === 'done';
  const allContainersDone = containerStatuses.every((c) => c.status === 'done');
  const isCreateDisabled = !allDaemonSetSectionsDone || !podDone || !allContainersDone;
          ...
          <div className="pt-4 px-8 pb-20 bg-[var(--color-surface-default)]">
            <VStack gap={6}>
              ...
              <VStack gap={2}>
                ...
                <p className="text-[11px] text-[var(--color-text-subtle)] leading-[16px]">
                  DaemonSets ensure that all (or some) nodes run a copy of a Pod, typically for system services like logging or monitoring.
                </p>
              </VStack>
              ...
              <HStack gap={6} align="start" className="w-full">
                ...
                <VStack gap={4} className="flex-1">
                  {activeTab === 'daemonset' && (
                    <>
                      ...
                      {daemonsetSectionStatus['basic-info'] === 'pre' && (
                        <PreSection title={DAEMONSET_SECTION_LABELS['basic-info']} />
                      )}
                      {daemonsetSectionStatus['basic-info'] === 'writing' && (
                        <WritingSection title={DAEMONSET_SECTION_LABELS['basic-info']} />
                      )}
                      {daemonsetSectionStatus['basic-info'] === 'active' && (
                        <BasicInfoSection
                          namespace={namespace}
                      ...
                      {daemonsetSectionStatus['basic-info'] === 'done' && (
                        <DoneSection
                          title={DAEMONSET_SECTION_LABELS['basic-info']}
                      ...
                      {daemonsetSectionStatus['labels-annotations'] === 'pre' && (
                        <PreSection title={DAEMONSET_SECTION_LABELS['labels-annotations']} />
                      )}
                      {daemonsetSectionStatus['labels-annotations'] === 'writing' && (
                        <WritingSection title={DAEMONSET_SECTION_LABELS['labels-annotations']} />
                      )}
                      {daemonsetSectionStatus['labels-annotations'] === 'active' && (
                      ...
                      {daemonsetSectionStatus['labels-annotations'] === 'done' && (
                        <DoneSection
                          title={DAEMONSET_SECTION_LABELS['labels-annotations']}
                      ...
                      {daemonsetSectionStatus['scaling-policy'] === 'pre' && (
                        <PreSection title={DAEMONSET_SECTION_LABELS['scaling-policy']} />
                      )}
                      {daemonsetSectionStatus['scaling-policy'] === 'writing' && (
                        <WritingSection title={DAEMONSET_SECTION_LABELS['scaling-policy']} />
                      )}
                      {daemonsetSectionStatus['scaling-policy'] === 'active' && (
                      ...
                      {daemonsetSectionStatus['scaling-policy'] === 'done' && (
                        <DoneSection
                          title={DAEMONSET_SECTION_LABELS['scaling-policy']}
                ...
                {/* Summary Sidebar */}
                <SummarySidebar
                  daemonsetSectionStatus={daemonsetSectionStatus}
...
export default CreateDaemonSetPage;
