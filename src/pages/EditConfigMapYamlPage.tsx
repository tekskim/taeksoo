...
const getMockYamlContent = (configMapName: string) => `apiVersion: v1
kind: ConfigMap
metadata:
  name: '${configMapName}'
  annotations:
...
export function EditConfigMapYamlPage() {
  ...
  useEffect(() => {
    if (configMapName) {
      ...
      setYamlContent(getMockYamlContent(configMapName));
    }
  }, [configMapName]);

  // Update tab label
  useEffect(() => {
    updateActiveTabLabel(`Edit ${configMapName || 'ConfigMap'} (YAML)`);
  }, [updateActiveTabLabel, configMapName]);
...
export default EditConfigMapYamlPage;
