...
export function OverallPerformancePage() {
  ...
  const [poolSelect, setPoolSelect] = useState('all');
  ...
  const osdLatencyColumns: TableColumn<OsdLatencyRow>[] = [
    { key: 'osdId', label: 'OSD ID', flex: 1, minWidth: columnMinWidths.osd, sortable: true },
    { key: 'latency', label: 'Latency', flex: 1, minWidth: columnMinWidths.type, sortable: true },
  ];
  ...
  const slowOsdOpsColumns: TableColumn<SlowOsdOpsRow>[] = [
    {
      ...
      render: (_, row) => (
        <Link
          to={`/storage/hosts/host-${row.osdId}`}
          className="font-medium text-[var(--color-action-primary)] hover:underline"
        >
          {row.osdId}
        </Link>
                ...
                <div className="flex items-center gap-2">
                  <Select
                    options={[
                      ...
                      { value: 'thaki', label: 'thaki' },
                      { value: 'hostway', label: 'Hostway' },
                      { value: 'bespin', label: 'BespinGlobal' },
                    ]}
                    value={poolSelect}
                    onChange={setPoolSelect}
                    size="sm"
                    className="w-[240px]"
                  />
                  <MonitoringToolbar
