    ...
    <div className="flex flex-col gap-3 w-full">
      ...
      <HStack justify="between" align="center">
        <span className="text-[11px] text-[var(--color-text-subtle)]">
          {hasChanges
            ? 'Showing differences from default values'
            : 'No changes from default values'}
        </span>
        <HStack gap={1}>
          <button
          ...
          <VStack gap={4} className="flex-1">
                    ...
                    <TabPanel value="options">
                      <div className="pt-4">
                        <OptionsForm
                          opts={opts}
                          values={optionValues}
                          onChange={(k, v) => setOptionValues((prev) => ({ ...prev, [k]: v }))}
                        />
                      </div>
                    </TabPanel>
                    <TabPanel value="yaml">
            ...
            {sectionStatus.configuration === 'done' && (
              <DoneSection
                ...
                onEdit={() => setStep({ configuration: 'active', compare: 'pre' })}
              >
                {opts.length > 0 ? (
                  opts
                    .slice(0, 3)
            ...
            {sectionStatus.compare === 'pre' && <PreSection title={SECTION_LABELS.compare} />}
            ...
            {sectionStatus.compare === 'done' && (
              <DoneSection
                title={SECTION_LABELS.compare}
                onEdit={() => setStep({ compare: 'active' })}
              >
