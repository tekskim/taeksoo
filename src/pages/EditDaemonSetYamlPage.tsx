...
const getMockYamlContent = (daemonSetName: string) => `apiVersion: v1
kind: DaemonSet
metadata:
  name: '${daemonSetName}'
  annotations:
...
export function EditDaemonSetYamlPage() {
  const navigate = useNavigate();
  const { daemonSetName } = useParams<{ daemonSetName: string }>();
  ...
  useEffect(() => {
    if (daemonSetName) {
      ...
      setYamlContent(getMockYamlContent(daemonSetName));
    }
  }, [daemonSetName]);

  // Update tab label
  useEffect(() => {
    updateActiveTabLabel(`Edit ${daemonSetName || 'DaemonSet'} (YAML)`);
  }, [updateActiveTabLabel, daemonSetName]);
  ...
  const handleCancel = useCallback(() => {
    navigate(`/container/daemonsets/${daemonSetName}`);
  }, [navigate, daemonSetName]);

  // Handle save
  const handleSave = useCallback(() => {
    // TODO: Implement actual daemonset update via API
    console.log('Saving daemonset YAML:', yamlContent);
    navigate(`/container/daemonsets/${daemonSetName}`);
  }, [navigate, daemonSetName, yamlContent]);
          ...
          breadcrumb={
            <Breadcrumb
              items={[
                ...
                { label: daemonSetName || 'DaemonSet', href: `/container/daemonsets/${daemonSetName}` },
                { label: 'Edit YAML' },
              ]}
            />
          }
...
export default EditDaemonSetYamlPage;
