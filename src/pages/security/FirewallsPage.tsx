...
export function FirewallsPage() {
  ...
  const breadcrumbItems = [{ label: 'Firewalls' }];
    ...
    },
    {
      ...
      render: (_, row) =>
        row.ingressPolicy ? (
          <div className="flex flex-col gap-0.5 min-w-0">
            <Link
              to={`/security/firewall-policies/${row.ingressPolicyId}`}
              className="text-label-md text-[var(--color-action-primary)] hover:underline hover:underline-offset-2"
              onClick={(e) => e.stopPropagation()}
            >
              {row.ingressPolicy}
    ...
    },
    {
      ...
      render: (_, row) =>
        row.egressPolicy ? (
          <div className="flex flex-col gap-0.5 min-w-0">
            <Link
              to={`/security/firewall-policies/${row.egressPolicyId}`}
              className="text-label-md text-[var(--color-action-primary)] hover:underline hover:underline-offset-2"
              onClick={(e) => e.stopPropagation()}
            >
              {row.egressPolicy}
            ...
            <Button
              ...
              onClick={() => {
                if (activeTab === 'firewalls') {
                  navigate('/security/firewalls/create');
                } else if (activeTab === 'policies') {
                  navigate('/security/firewalls/create-policy');
                } else {
                  navigate('/security/firewalls/create-rule');
                }
              }}
            >
              {activeTab === 'firewalls'
