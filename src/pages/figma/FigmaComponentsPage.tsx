...
const Section = ({ id, children }: { id: string; children: React.ReactNode }) => (
  <section data-figma-name={`[Section] ${id}`} className="flex flex-col pt-[80px]">
    {children}
  </section>
);
...
function TabBarDemo({ showWindowControls = true }: { showWindowControls?: boolean }) {
  const [activeTab, setActiveTab] = useState('tab1');
  const tabs = [
    { id: 'tab1', label: 'Overview', closable: false },
    { id: 'tab2', label: 'Instances', closable: true },
    { id: 'tab3', label: 'Pods', closable: true },
    { id: 'tab4', label: 'Settings', closable: true },
  ];
  return (
    <div className="w-[720px] border border-[var(--color-border-default)] rounded-t-[var(--primitive-radius-lg)] overflow-hidden">
      <TabBar
        tabs={tabs}
        activeTab={activeTab}
    ...
    <div className="flex flex-col">
      ...
      <Section id="Button">
        ...
        {buttonVariants.map((variant) => {
          ...
          const hasIconLeft = variant !== 'link';
          const hasIconRight = variant === 'primary';
          const hasIconOnly = variant === 'secondary';
          ...
          return (
            <React.Fragment key={variant}>
              ...
              <div
                ...
                style={{ gridTemplateColumns: '72px repeat(4, auto)' }}
              >
                <div />
                {buttonStates.map((s) => (
                  <StateLabel key={s}>{s}</StateLabel>
                ))}

                {buttonSizes.map((size) => (
                  <React.Fragment key={`text-${size}`}>
                    <StateLabel>{size}</StateLabel>
                    {buttonStates.map((state) => (
                      <FigmaFrame key={state} name={`Button/${variant}/${size}/${state}`}>
                        <Button
                          variant={variant}
                          size={size}
                          disabled={state === 'disabled'}
                          isLoading={state === 'loading'}
                          className={state === 'hover' ? buttonHoverClasses[variant] : undefined}
                          aria-label={`Button/${variant}/${size}/${state}`}
                        >
                          {variant}
                        </Button>
                      </FigmaFrame>
                ...
                {hasIconLeft &&
                  buttonSizes.map((size) => (
                    <React.Fragment key={`icon-left-${size}`}>
                      <StateLabel>{size} / icon</StateLabel>
                      {buttonStates.map((state) => (
                        <FigmaFrame
                          key={state}
                          name={`Button/${variant}/${size}/icon-left/${state}`}
                        >
                          <Button
                            variant={variant}
                            size={size}
                            ...
                            isLoading={state === 'loading'}
                            className={state === 'hover' ? buttonHoverClasses[variant] : undefined}
                            aria-label={`Button/${variant}/${size}/icon-left/${state}`}
                          >
                            {iconLabel}
                          </Button>
                        </FigmaFrame>
                ...
                {hasIconRight &&
                  buttonSizes.map((size) => (
                    <React.Fragment key={`icon-right-${size}`}>
                      <StateLabel>{size} / right</StateLabel>
                      <FigmaFrame name={`Button/primary/${size}/icon-right/default`}>
                        <Button
                          variant="primary"
                          size={size}
                          rightIcon={<IconChevronDown size={size === 'lg' ? 14 : 12} />}
                          aria-label={`Button/primary/${size}/icon-right/default`}
                        >
                          Dropdown
                        </Button>
                      </FigmaFrame>
                      <FigmaFrame name={`Button/primary/${size}/icon-right/hover`}>
                        <Button
                          variant="primary"
                          size={size}
                          rightIcon={<IconChevronDown size={size === 'lg' ? 14 : 12} />}
                          className={buttonHoverClasses.primary}
                          aria-label={`Button/primary/${size}/icon-right/hover`}
                        >
                          Dropdown
                        </Button>
                      </FigmaFrame>
                ...
                {hasIconOnly &&
                  buttonSizes.map((size) => (
                    <React.Fragment key={`icon-only-${size}`}>
                      <StateLabel>{size} / only</StateLabel>
                      <FigmaFrame name={`Button/secondary/${size}/icon-only/default`}>
                        <Button
                          variant="secondary"
                          size={size}
                          icon={<IconDownload size={12} />}
                          aria-label={`Button/secondary/${size}/icon-only/default`}
                        />
                      </FigmaFrame>
                      <FigmaFrame name={`Button/secondary/${size}/icon-only/hover`}>
                        <Button
                          variant="secondary"
                          size={size}
                          icon={<IconDownload size={12} />}
                          aria-label={`Button/secondary/${size}/icon-only/hover`}
      ...
      <Section id="Input">
        ...
        <div
          className="grid items-end gap-x-3 gap-y-2"
          style={{ gridTemplateColumns: '40px repeat(5, auto)' }}
        >
          <div />
      ...
      <Section id="NumberInput">
        ...
        >
          ...
          <StateLabel>arrow-hover</StateLabel>
          <StateLabel>suffix</StateLabel>
          <StateLabel>disabled</StateLabel>
      ...
      <Section id="Badge">
        ...
        <div
          className="grid items-end gap-x-3 gap-y-2"
          style={{ gridTemplateColumns: `40px repeat(${badgeThemes.length}, auto)` }}
        >
          <div />
          {badgeThemes.map((theme) => (
            <StateLabel key={theme}>{theme}</StateLabel>
          ))}
      ...
      <Section id="Chip">
        ...
        <div
          ...
          style={{ gridTemplateColumns: 'repeat(4, auto)' }}
        >
          <StateLabel>value-only/default</StateLabel>
          <StateLabel>value-only/removable</StateLabel>
          <StateLabel>selected-removable</StateLabel>
          <StateLabel>label-value/removable</StateLabel>
          ...
          <FigmaFrame name="Chip/selected-removable">
        ...
        <FigmaGuide
          ...
          tips={[
            'Value only: 단순 표시(NotificationCenter) 또는 태그 삭제(CreateMCPTemplate)',
            'Selected + Removable: SelectionIndicator 내부에서 선택 결과 표시',
            'Label + Value + Removable: FilterSearchInput에서 필터 칩으로 사용',
            'Badge와 용도 구분: Chip은 인터랙티브(선택/삭제 가능), Badge는 읽기 전용',
          ]}
        />
      </Section>

      <Section id="StatusIndicator">
        {/* ════════════════ STATUS INDICATOR ════════════════ */}
        <SectionTitle>StatusIndicator</SectionTitle>
        <SubTitle>All Statuses (icon-only)</SubTitle>
        <div
          className="grid items-end gap-x-3 gap-y-2"
          style={{ gridTemplateColumns: `repeat(${statusTypes.length}, auto)` }}
        >
          {statusTypes.map((status) => (
            <StateLabel key={`hdr-${status}`}>{status}</StateLabel>
          ))}
          {statusTypes.map((status) => (
        ...
        <FigmaGuide
          ...
          properties={[
            ...
            { name: 'Layout', type: 'Variant', values: 'icon-only' },
          ]}
          autoLayout={[{ label: 'icon-only', direction: 'H', gap: '0', padding: '4px 6px' }]}
          radius="9999px (pill)"
          tokens={[
          ...
          tips={[
            ...
            'icon-only: 원형(pill) 배경에 색상만 표시. 테이블 셀, DetailHeader에서 사용',
      ...
      <Section id="Pagination">
        ...
        <div className="flex flex-col gap-4">
          <FigmaFrame name="Pagination/first-page">
            <Pagination
              currentPage={1}
              totalPages={10}
              onPageChange={() => {}}
          ...
          <FigmaFrame name="Pagination/with-selection">
            <Pagination
              currentPage={1}
              totalPages={10}
              onPageChange={() => {}}
        ...
        >
          <StateLabel>Prev/disabled</StateLabel>
          <StateLabel>Prev/enabled</StateLabel>
          <StateLabel>Next/enabled</StateLabel>
          <StateLabel>Page/default</StateLabel>
          <StateLabel>Page/active</StateLabel>
          <StateLabel>Dots</StateLabel>
          <StateLabel>Settings</StateLabel>
          <StateLabel>ItemCount</StateLabel>
          <StateLabel>SelectedCount</StateLabel>
      ...
      <Section id="ProgressBar">
        ...
        >
          ...
          <StateLabel>25%</StateLabel>
          <StateLabel>50%</StateLabel>
          <StateLabel>75%</StateLabel>
      ...
      <Section id="Tabs">
        ...
        >
          ...
          <StateLabel>boxed</StateLabel>

          <FigmaFrame name="Tabs/underline">
            <Tabs value="tab1" onChange={() => {}} variant="underline" size="sm">
              ...
              <TabPanel value="tab1">
                <div className="p-4 text-body-md text-[var(--color-text-muted)]">
                  Tab content area
                </div>
              </TabPanel>
            </Tabs>
          </FigmaFrame>
          <FigmaFrame name="Tabs/boxed">
        ...
        >
          ...
          <StateLabel>hover</StateLabel>
          <StateLabel>indicator</StateLabel>
      ...
      <Section id="Breadcrumb">
        ...
        >
          ...
          <StateLabel>separator</StateLabel>
      ...
      <Section id="InlineMessage">
        {/* ════════════════ INLINE MESSAGE ════════════════ */}
        <SectionTitle>InlineMessage</SectionTitle>
        <div
          className="grid items-start gap-x-3 gap-y-2"
          style={{ gridTemplateColumns: `repeat(${inlineMessageVariants.length}, 1fr)` }}
        >
          {inlineMessageVariants.map((variant) => (
            <StateLabel key={`hdr-${variant}`}>{variant}</StateLabel>
          ))}
          {inlineMessageVariants.map((variant) => (
      ...
      <Section id="FormField">
        ...
        >
          ...
          <StateLabel>description + helper</StateLabel>
          <StateLabel>error</StateLabel>
        ...
        <div
          ...
          style={{ gridTemplateColumns: 'repeat(5, auto)' }}
        >
          <StateLabel>label/default</StateLabel>
          <StateLabel>label/required</StateLabel>
          <StateLabel>description</StateLabel>
          <StateLabel>helper text</StateLabel>
          <StateLabel>error message</StateLabel>
      ...
      <Section id="SectionCard">
        ...
        <FigmaGuide
          ...
          autoLayout={[
            { label: 'Default', direction: 'V', gap: '12px', padding: 'pt-3 px-4 pb-6' },
          ]}
      ...
      <Section id="PageHeader">
        ...
        <FigmaFrame name="PageHeader/title-only">
        ...
        <FigmaGuide
          ...
          autoLayout={[
            { label: 'Default', direction: 'H', gap: '—', padding: '0', height: '32px' },
          ]}
          tips={[
            '구조: 좌측 Title | 우측 Actions(Button). 수평 Auto Layout, space-between',
            'Title: heading-h5 (16px semibold)',
      ...
      <Section id="ListToolbar">
        {/* ════════════════ LIST TOOLBAR ════════════════ */}
        <SectionTitle>ListToolbar</SectionTitle>
        <FigmaFrame name="ListToolbar/search-only">
          <ListToolbar
            primaryActions={
              <ListToolbar.Actions>
                <FilterSearchInput
        ...
        <FigmaFrame name="ListToolbar/with-filters">
          <ListToolbar
            primaryActions={
              <ListToolbar.Actions>
                <FilterSearchInput
            ...
            filters={[
              { id: 'f1', label: 'Status', value: 'Running' },
              { id: 'f2', label: 'Name', value: 'web-server' },
            ]}
            onFilterRemove={() => {}}
            onFiltersClear={() => {}}
          />
      ...
      <Section id="Tooltip">
        ...
        <div
          className="grid items-end gap-x-8 gap-y-2"
          style={{ gridTemplateColumns: 'repeat(4, auto)' }}
        >
          <StateLabel>top</StateLabel>
          <StateLabel>bottom</StateLabel>
          <StateLabel>left</StateLabel>
          <StateLabel>right</StateLabel>

          <FigmaFrame name="Tooltip/top">
            <div className="flex flex-col items-center gap-2">
              <div className="relative">
                <div className="bg-[var(--tooltip-bg)] text-[var(--tooltip-text)] px-[var(--tooltip-padding-x)] py-[var(--tooltip-padding-y)] rounded-[var(--tooltip-radius)] text-[length:var(--tooltip-font-size)] leading-[var(--tooltip-line-height)] text-center w-max">
                  Top tooltip
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-full w-0 h-0 border-[length:var(--tooltip-arrow-size)] border-solid border-l-transparent border-r-transparent border-b-transparent border-t-[var(--tooltip-bg)]" />
              </div>
            </div>
          </FigmaFrame>
          <FigmaFrame name="Tooltip/bottom">
            <div className="flex flex-col items-center gap-2">
              <div className="relative">
                <div className="absolute left-1/2 -translate-x-1/2 top-0 -translate-y-full w-0 h-0 border-[length:var(--tooltip-arrow-size)] border-solid border-l-transparent border-r-transparent border-t-transparent border-b-[var(--tooltip-bg)]" />
                <div className="bg-[var(--tooltip-bg)] text-[var(--tooltip-text)] px-[var(--tooltip-padding-x)] py-[var(--tooltip-padding-y)] rounded-[var(--tooltip-radius)] text-[length:var(--tooltip-font-size)] leading-[var(--tooltip-line-height)] text-center w-max">
                  Bottom tooltip
                </div>
              </div>
          ...
          <FigmaFrame name="Tooltip/left">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <div className="absolute right-0 translate-x-full top-1/2 -translate-y-1/2 w-0 h-0 border-[length:var(--tooltip-arrow-size)] border-solid border-t-transparent border-b-transparent border-r-transparent border-l-[var(--tooltip-bg)]" />
              </div>
            </div>
          </FigmaFrame>
          <FigmaFrame name="Tooltip/right">
            <div className="flex items-center gap-2">
              <div className="relative">
                <div className="absolute left-0 -translate-x-full top-1/2 -translate-y-1/2 w-0 h-0 border-[length:var(--tooltip-arrow-size)] border-solid border-t-transparent border-b-transparent border-l-transparent border-r-[var(--tooltip-bg)]" />
                <div className="bg-[var(--tooltip-bg)] text-[var(--tooltip-text)] px-[var(--tooltip-padding-x)] py-[var(--tooltip-padding-y)] rounded-[var(--tooltip-radius)] text-[length:var(--tooltip-font-size)] leading-[var(--tooltip-line-height)] text-center w-max">
                  Right tooltip
                </div>
              </div>
          ...
          <FigmaFrame name="Popover/badge-list-overflow">
            <div className="flex flex-col items-center">
              ...
              <div className="relative">
                ...
                <div
                  className="bg-[var(--color-surface-default)] border border-[var(--color-border-default)] rounded-[var(--primitive-radius-lg)] shadow-lg"
                ...
                >
                  <div className="p-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <Badge size="sm" className="w-fit max-w-full">
                        <span className="break-all">app=nginx</span>
                      </Badge>
                      <Badge size="sm" className="w-fit max-w-full">
                        <span className="break-all">env=production</span>
                      </Badge>
                      <Badge size="sm" className="w-fit max-w-full">
                        <span className="break-all">tier=frontend</span>
                      </Badge>
                      <Badge size="sm" className="w-fit max-w-full">
                        <span className="break-all">version=v2.1.0</span>
                      </Badge>
                      <Badge size="sm" className="w-fit max-w-full">
                        <span className="break-all">team=platform</span>
                      </Badge>
                    </div>
                  </div>
                </div>
      ...
      <Section id="TabBar">
        ...
        <div className="flex flex-col gap-4">
          <FigmaFrame name="TabBar/with-window-controls">
            <TabBarDemo showWindowControls />
          </FigmaFrame>
          <FigmaFrame name="TabBar/without-window-controls">
            <TabBarDemo showWindowControls={false} />
          </FigmaFrame>
        </div>
        ...
        <div
          ...
          style={{ gridTemplateColumns: 'repeat(6, auto)' }}
        >
          <StateLabel>Tab/active</StateLabel>
          <StateLabel>Tab/inactive</StateLabel>
          <StateLabel>CloseButton</StateLabel>
          <StateLabel>ActiveIndicator</StateLabel>
          <StateLabel>AddButton</StateLabel>
          <StateLabel>WindowControls</StateLabel>
        ...
        <div
          className="grid items-end gap-x-3 gap-y-2 mt-2"
          style={{ gridTemplateColumns: 'repeat(3, auto)' }}
        >
          <StateLabel>minimize</StateLabel>
          <StateLabel>maximize</StateLabel>
      ...
      <Section id="TopBar">
        ...
        >
          <StateLabel>SidebarToggle</StateLabel>
          <StateLabel>Navigation</StateLabel>
          <StateLabel>Back</StateLabel>
          <StateLabel>Forward</StateLabel>
          <StateLabel>Action/default</StateLabel>
          <StateLabel>Action/badge</StateLabel>
          <StateLabel>Action/active</StateLabel>
          ...
          <FigmaFrame name="TopBar/Sub/Navigation">
            <div className="flex items-center gap-0.5">
              <button className="size-[var(--topbar-button-size,28px)] flex items-center justify-center rounded-[var(--topbar-button-radius,6px)] hover:bg-[var(--color-surface-hover)]">
                <IconArrowLeft size={12} stroke={1.5} className="text-[var(--color-text-muted)]" />
              </button>
              <button className="size-[var(--topbar-button-size,28px)] flex items-center justify-center rounded-[var(--topbar-button-radius,6px)] hover:bg-[var(--color-surface-hover)]">
                <IconArrowRight size={12} stroke={1.5} className="text-[var(--color-text-muted)]" />
              </button>
            </div>
          </FigmaFrame>
      ...
      <Section id="Disclosure">
        ...
        >
          ...
          <StateLabel>expanded</StateLabel>
      ...
      <Section id="CopyButton">
        ...
        >
          <StateLabel>default</StateLabel>
          <StateLabel>truncate</StateLabel>
      ...
      <Section id="SelectionIndicator">
        ...
        >
          <StateLabel>empty</StateLabel>
          <StateLabel>with-selection</StateLabel>
          <StateLabel>error</StateLabel>
      ...
      <Section id="WindowControl">
        ...
        >
          ...
          <StateLabel>maximize</StateLabel>
          <StateLabel>close</StateLabel>
          <StateLabel>minimize/hover</StateLabel>
          <StateLabel>maximize/hover</StateLabel>
          <StateLabel>close/hover</StateLabel>
          ...
          <FigmaFrame name="WindowControl/minimize/hover">
            <button
              type="button"
          ...
          <FigmaFrame name="WindowControl/maximize/hover">
            <button
              type="button"
              className="flex items-center justify-center w-[var(--window-control-size)] h-[var(--window-control-size)] rounded-[var(--window-control-radius)] text-[var(--color-text-default)] bg-[var(--color-surface-subtle)]"
              aria-label="maximize"
