...
function YamlViewer({ value, onCopy, onDownload }: { value: string; onCopy: () => void; onDownload: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-2 w-full min-h-0 flex-1">
      <HStack justify="end" gap={2}>
        <Button variant="secondary" size="sm" leftIcon={<IconCopy size={12} stroke={1.5} />} onClick={onCopy}>
          Copy
        </Button>
        <Button variant="secondary" size="sm" leftIcon={<IconDownload size={12} stroke={1.5} />} onClick={onDownload}>
          Download
        </Button>
      </HStack>
    ...
    return (
      ...
      >
        <VStack gap={4}>
          ...
          <Button variant="secondary" size="sm" onClick={() => navigate('/container/apps/installed-apps')}>
      ...
      <VStack gap={6}>
        <DetailHeader>
          ...
          <DetailHeader.InfoGrid>
            ...
            <DetailHeader.InfoCard label="Last deployed" value={app.lastDeployed ?? app.installedAt ?? '—'} />
          </DetailHeader.InfoGrid>
        </DetailHeader>
            ...
            <VStack gap={3} className="pt-3">
              {(app.resources ?? []).length === 0 ? (
                ...
                <Table
                  ...
                  data={(app.resources ?? []).map((r, i) => ({ ...r, id: `${r.kind}-${r.name}-${i}` }))}
                  rowKey="id"
                />
              )}
            </VStack>
