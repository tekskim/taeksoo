...
const getMockYamlContent = (limitRangeName: string) => `apiVersion: v1
kind: LimitRange
metadata:
  name: '${limitRangeName}'
  annotations:
...
export function EditLimitRangeYamlPage() {
  const navigate = useNavigate();
  const { limitRangeName } = useParams<{ limitRangeName: string }>();
  ...
  // Load YAML content based on limitRange ID
  useEffect(() => {
    if (limitRangeName) {
      // TODO: Fetch actual YAML from API
      setYamlContent(getMockYamlContent(limitRangeName));
    }
  }, [limitRangeName]);

  // Update tab label
  useEffect(() => {
    updateActiveTabLabel(`Edit ${limitRangeName || 'LimitRange'} (YAML)`);
  }, [updateActiveTabLabel, limitRangeName]);
  ...
  const handleCancel = useCallback(() => {
    navigate(`/container/limitRanges/${limitRangeName}`);
  }, [navigate, limitRangeName]);

  // Handle save
  const handleSave = useCallback(() => {
    // TODO: Implement actual limitRange update via API
    console.log('Saving limitRange YAML:', yamlContent);
    navigate(`/container/limitRanges/${limitRangeName}`);
  }, [navigate, limitRangeName, yamlContent]);
          ...
          breadcrumb={
            <Breadcrumb
              items={[
                ...
                { label: limitRangeName || 'LimitRange', href: `/container/limitRanges/${limitRangeName}` },
                { label: 'Edit YAML' },
              ]}
            />
          }
...
export default EditLimitRangeYamlPage;
