...
const statusIndicatorProps: PropDef[] = [
  {
    name: 'status',
    type: "'active' | 'error' | 'building' | 'deleting' | 'suspended' | 'shutoff' | 'paused' | 'pending' | ...",
    required: true,
  ...
  },
  {
    ...
    default: "'md'",
    required: false,
    description: 'Size (icon-only layout)',
  },
...
export function StatusIndicatorPage() {
  return (
    <ComponentPageTemplate
      ...
      preview={
        <ComponentPreview
          code={`<StatusIndicator status="active" />
...
<StatusIndicator status="error" label="Failed" />`,
          ...
          <VStack gap={4}>
            ...
            <VStack gap={2}>
              ...
              <ul className="list-disc pl-5 text-body-sm text-[var(--color-text-muted)] space-y-1">
                <li>
                  테이블의 Status 컬럼에서 사용하며, 컬럼은 <strong>중앙 정렬</strong> 합니다.
                </li>
      ...
      accessibility={
        <p className="text-body-md text-[var(--color-text-muted)]">
          StatusIndicator uses semantic colors and aria attributes. Use with Tooltip for icon-only
          layout to convey status to screen readers.
        </p>
      }
      relatedLinks={[
