...
import { VStack, HStack, EmptyState, Button } from '@/design-system';
import {
  IconDatabase,
  IconSearch,
  IconFolder,
...
export function EmptyStatesPage() {
  return (
    <ComponentPageTemplate
      ...
      preview={
        <ComponentPreview
          code={`// variant="card" (기본) — 초기 상태, 생성 유도
<EmptyState
          ...
          <VStack gap={6} className="w-full">
            <HStack gap={6} className="w-full" align="start">
              ...
              <VStack gap={2} className="flex-1">
                ...
                <div className="border border-dashed border-[var(--color-border-default)] rounded-[var(--primitive-radius-lg)]">
                  <EmptyState
                    variant="inline"
                    icon={<IconSearch size={48} stroke={1} />}
                    title="No results found"
        ...
        <VStack gap={8}>
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              ...
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                데이터가 아직 없는 초기 상태. 리소스 타입에 맞는 아이콘 + &quot;No [리소스]
                found&quot; 제목 + 생성 유도 액션 버튼.
              </span>
            </VStack>
            <EmptyState
              icon={<IconDatabase size={48} stroke={1} />}
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              ...
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                검색어나 필터 조건에 해당하는 결과가 없을 때. 검색 아이콘 사용, 필터 초기화 보조
                액션 제공 권장.
              </span>
            </VStack>
            <EmptyState
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              ...
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                파일이 아직 없는 상태. 리소스에 맞는 아이콘과 업로드 액션 제공.
              </span>
            </VStack>
            <EmptyState
              ...
              action={
                <Button variant="primary" size="md" leftIcon={<IconUpload size={12} />}>
                  Upload file
                </Button>
              }
            />
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              ...
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                테이블이나 섹션 내부에서 사용. 테두리 없이 패딩만 적용되는
                variant=&quot;inline&quot;.
              </span>
            </VStack>
            <div className="border border-[var(--color-border-default)] rounded-[var(--primitive-radius-lg)]">
              <EmptyState
                variant="inline"
                icon={<IconNetwork size={48} stroke={1} />}
                title="No networks found"
                description="Create a network to connect your instances."
              />
            </div>
          </VStack>
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              <span className="text-label-md text-[var(--color-text-default)]">권한 없음 상태</span>
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                권한이 없어 데이터에 접근할 수 없는 경우. 액션 버튼을 노출하지 않고, 권한 요청
                방법을 안내하는 설명으로 대체.
              </span>
            </VStack>
            <EmptyState
              icon={<IconLock size={48} stroke={1} />}
              title="No access"
          ...
          <VStack gap={3}>
            <VStack gap={1}>
              ...
              <span className="text-body-sm text-[var(--color-text-subtle)]">
                아이콘이 상태 전달에 기여하지 않는 경우 생략 가능. 제목과 설명만으로 구성.
              </span>
            </VStack>
            <EmptyState
              title="No volumes attached"
              description="Attach a volume to this instance to enable persistent storage."
            />
          </VStack>
        </VStack>
      }
