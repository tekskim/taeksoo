...
const PREV_VERSION_MARKDOWN = `
## 1. 개요

Chart는 읽기 전용(Read-only) 시각화 요소이다. 사용자가 데이터를 직접 수정하거나 조작하지 않는다.

## 2. 공통 규칙

### 색상 상태
- Safe: 0~69% (green)
- Warning: 70~94% (orange)
- Danger: 95%+ (red)

### 텍스트 표시
- 차트 내 텍스트는 가독성을 위해 간결하게 표시한다.

## 3. 차트 유형별 정책

### Half doughnut chart
- **구성요소**: 중앙 값, arc(사용률), 색상 상태
- **호버정책**: Arc hover 시 tooltip에 값 표시

### Bar chart
- **구성요소**: Bar(사용률), 색상 상태, 라벨
- **호버정책**: Bar hover 시 tooltip에 값 표시

### Line chart
- **구성요소**: X축(시간), Y축(값), 라인, 데이터 포인트
- **호버정책**: 데이터 포인트 hover 시 tooltip에 해당 시점 값 표시
- **X축**: 시간 단위, 동일 차트 내 동일 단위 유지
- **Y축**: 단일 단위, tick 5개

### Pie chart
- **구성요소**: 슬라이스, 비율, Legend
- **호버정책**: 슬라이스 hover 시 tooltip에 값/비율 표시
`;
      ...
      guidelines={
        <>
          ...
          <Disclosure className="mt-6">
            ...
            <Disclosure.Panel>
              <div className="pt-2">
                <NotionRenderer markdown={PREV_VERSION_MARKDOWN} />
              </div>
            </Disclosure.Panel>
          </Disclosure>
        </>
