...
type StatefulSetSectionStep = 'basic-info' | 'labels-annotations' | 'scaling-policy';
...
const STATEFULSET_SECTION_LABELS: Record<StatefulSetSectionStep, string> = {
...
const STATEFULSET_SECTION_ORDER: StatefulSetSectionStep[] = [
  'basic-info',
  'labels-annotations',
  'scaling-policy',
];
...
interface SummarySidebarProps {
  statefulsetSectionStatus: Record<StatefulSetSectionStep, SectionState>;
...
function SummarySidebar({ 
  ...
  const [statefulsetExpanded, setStatefulsetExpanded] = useState(true);
  ...
  // Check if all statefulset sections are done
  const allStatefulSetDone = STATEFULSET_SECTION_ORDER.every(
    (key) => statefulsetSectionStatus[key] === 'done'
  );
          ...
          <VStack gap={3}>
            ...
            <VStack gap={0}>
              ...
              <HStack 
                ...
                onClick={() => setStatefulsetExpanded(!statefulsetExpanded)}
              >
                ...
                {allStatefulSetDone && <WizardSectionStatusIcon status="done" />}
              </HStack>

              {/* StatefulSet Sub-items (Indented) */}
              {statefulsetExpanded && (
                <VStack gap={0} className="pl-4">
                  {STATEFULSET_SECTION_ORDER.map((key) => {
                    const status = mapState(statefulsetSectionStatus[key]);
                    return (
                      <HStack 
                        key={key}
                        justify="between" 
                      ...
                      >
                        <span className="text-[length:var(--font-size-12)] leading-[var(--line-height-18)] text-[var(--color-text-default)]">
                          {STATEFULSET_SECTION_LABELS[key]}
                        </span>
                        {status === 'writing' ? (
          ...
          {strategy === 'rolling-update' && (
            <VStack gap={4}>
              ...
              <VStack gap={2} className="max-w-[calc(50%-8px)]">
                ...
                <p className="text-[11px] text-[var(--color-text-subtle)] leading-[16px]">
                  The maximum time allowed for a StatefulSet to progress before it is considered failed.
                </p>
                <HStack gap={2}>
                  <NumberInput
...
export function CreateStatefulSetPage() {
  ...
  // Active tab (StatefulSet, Pod, Container-X)
  ...
  // Section states for StatefulSet tab
  const [statefulsetSectionStatus, setStatefulsetSectionStatus] = useState<Record<StatefulSetSectionStep, SectionState>>({
    'basic-info': 'active',
  ...
  // Editing state
  const [editingSection, setEditingSection] = useState<StatefulSetSectionStep | null>(null);
  ...
  const handleNext = useCallback((currentSection: StatefulSetSectionStep) => {
    const currentIndex = STATEFULSET_SECTION_ORDER.indexOf(currentSection);
    const nextSection = STATEFULSET_SECTION_ORDER[currentIndex + 1];

    setStatefulsetSectionStatus((prev) => ({
      ...prev,
      [currentSection]: 'done',
      ...(nextSection && { [nextSection]: 'active' }),
    }));

    // If this was the last statefulset section, mark Pod as active
    if (!nextSection) {
      setPodStatus('done');
      setContainerStatuses((prev) => 
  ...
  // Handle edit
  const handleEdit = useCallback((section: StatefulSetSectionStep) => {
    setEditingSection(section);
    const sectionIndex = STATEFULSET_SECTION_ORDER.indexOf(section);

    setStatefulsetSectionStatus((prev) => {
      const newStatus = { ...prev };
      STATEFULSET_SECTION_ORDER.forEach((key, index) => {
  ...
  const handleEditCancel = useCallback(() => {
    ...
    setStatefulsetSectionStatus((prev) => {
      ...
      const editIndex = STATEFULSET_SECTION_ORDER.indexOf(editingSection);
      for (let i = editIndex + 1; i < STATEFULSET_SECTION_ORDER.length; i++) {
        if (newStatus[STATEFULSET_SECTION_ORDER[i]] === 'writing') {
          newStatus[STATEFULSET_SECTION_ORDER[i]] = 'done';
        }
      }
      return newStatus;
    });
  ...
  const allStatefulSetSectionsDone = STATEFULSET_SECTION_ORDER.every(
    (key) => statefulsetSectionStatus[key] === 'done'
  );
  const podDone = podStatus === 'done';
  const allContainersDone = containerStatuses.every((c) => c.status === 'done');
  const isCreateDisabled = !allStatefulSetSectionsDone || !podDone || !allContainersDone;
          ...
          <div className="pt-4 px-8 pb-20 bg-[var(--color-surface-default)]">
            <VStack gap={6}>
              ...
              <VStack gap={2}>
                ...
                <p className="text-[11px] text-[var(--color-text-subtle)] leading-[16px]">
                  StatefulSets manage the deployment and scaling of stateful Pods, providing stable network identities and persistent storage.
                </p>
              </VStack>
              ...
              <HStack gap={6} align="start" className="w-full">
                ...
                <VStack gap={4} className="flex-1">
                  {activeTab === 'statefulset' && (
                    <>
                      ...
                      {statefulsetSectionStatus['basic-info'] === 'pre' && (
                        <PreSection title={STATEFULSET_SECTION_LABELS['basic-info']} />
                      )}
                      {statefulsetSectionStatus['basic-info'] === 'writing' && (
                        <WritingSection title={STATEFULSET_SECTION_LABELS['basic-info']} />
                      )}
                      {statefulsetSectionStatus['basic-info'] === 'active' && (
                        <BasicInfoSection
                          namespace={namespace}
                      ...
                      {statefulsetSectionStatus['basic-info'] === 'done' && (
                        <DoneSection
                          title={STATEFULSET_SECTION_LABELS['basic-info']}
                      ...
                      {statefulsetSectionStatus['labels-annotations'] === 'pre' && (
                        <PreSection title={STATEFULSET_SECTION_LABELS['labels-annotations']} />
                      )}
                      {statefulsetSectionStatus['labels-annotations'] === 'writing' && (
                        <WritingSection title={STATEFULSET_SECTION_LABELS['labels-annotations']} />
                      )}
                      {statefulsetSectionStatus['labels-annotations'] === 'active' && (
                      ...
                      {statefulsetSectionStatus['labels-annotations'] === 'done' && (
                        <DoneSection
                          title={STATEFULSET_SECTION_LABELS['labels-annotations']}
                      ...
                      {statefulsetSectionStatus['scaling-policy'] === 'pre' && (
                        <PreSection title={STATEFULSET_SECTION_LABELS['scaling-policy']} />
                      )}
                      {statefulsetSectionStatus['scaling-policy'] === 'writing' && (
                        <WritingSection title={STATEFULSET_SECTION_LABELS['scaling-policy']} />
                      )}
                      {statefulsetSectionStatus['scaling-policy'] === 'active' && (
                      ...
                      {statefulsetSectionStatus['scaling-policy'] === 'done' && (
                        <DoneSection
                          title={STATEFULSET_SECTION_LABELS['scaling-policy']}
                ...
                {/* Summary Sidebar */}
                <SummarySidebar
                  statefulsetSectionStatus={statefulsetSectionStatus}
...
export default CreateStatefulSetPage;
