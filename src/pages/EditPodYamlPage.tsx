...
const getMockYamlContent = (podName: string) => `apiVersion: v1
kind: Pod
metadata:
  name: '${podName}'
  annotations:
...
export function EditPodYamlPage() {
  ...
  // Load YAML content based on pod ID
  useEffect(() => {
    if (podName) {
      // TODO: Fetch actual YAML from API
      setYamlContent(getMockYamlContent(podName));
    }
  }, [podName]);
  ...
  const handleCancel = useCallback(() => {
    navigate(`/container/pods/${podName}`);
  }, [navigate, podName]);

  // Handle save
  const handleSave = useCallback(() => {
    // TODO: Implement actual pod update via API
    console.log('Saving pod YAML:', yamlContent);
    navigate(`/container/pods/${podName}`);
  }, [navigate, podName, yamlContent]);
    ...
    >
      <VStack gap={6} className="flex-1 min-h-0">
        ...
        <VStack gap={2} className="flex-shrink-0">
          <h1 className="text-heading-h5 text-[var(--color-text-default)]">Pod: {podName}</h1>
        </VStack>
...
export default EditPodYamlPage;
