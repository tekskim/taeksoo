...
type JobSectionStep = 'basic-info' | 'labels-annotations' | 'scaling-policy';
type PodSectionStep =
  | 'pod-labels'
  | 'pod-scaling'
  | 'pod-networking'
...
const JOB_SECTION_LABELS: Record<JobSectionStep, string> = {
...
const JOB_SECTION_ORDER: JobSectionStep[] = ['basic-info', 'labels-annotations', 'scaling-policy'];
...
interface SummarySidebarProps {
  jobSectionStatus: Record<JobSectionStep, SectionState>;
...
function SummarySidebar({
  ...
  // Check if all job sections are done
  const allJobDone = JOB_SECTION_ORDER.every((key) => jobSectionStatus[key] === 'done');
      ...
      <div className="bg-[var(--color-surface-default)] border border-[var(--color-border-default)] rounded-lg p-4 flex flex-col gap-6">
        ...
        <div className="bg-[var(--color-surface-subtle)] border border-[var(--color-border-default)] rounded-lg p-4">
          <VStack gap={3}>
            ...
            <VStack gap={0}>
              {/* Job Section (Collapsible Parent) */}
              <HStack
                justify="between"
                align="center"
              ...
              >
                ...
                {allJobDone && <WizardSectionStatusIcon status="done" />}
              </HStack>

              {/* Job Sub-items (Indented) */}
              {jobExpanded && (
                <VStack gap={0} className="pl-4">
                  {JOB_SECTION_ORDER.map((key) => {
...
export function CreateJobPage() {
  ...
  // Active tab (Job, Pod, Container-X)
  ...
  // Section states for Job tab
  const [jobSectionStatus, setJobSectionStatus] = useState<Record<JobSectionStep, SectionState>>({
    'basic-info': 'active',
  ...
  // Editing state
  const [editingSection, setEditingSection] = useState<JobSectionStep | null>(null);
  ...
  const handleNext = useCallback((currentSection: JobSectionStep) => {
    const currentIndex = JOB_SECTION_ORDER.indexOf(currentSection);
    const nextSection = JOB_SECTION_ORDER[currentIndex + 1];

    setJobSectionStatus((prev) => ({
      ...prev,
      [currentSection]: 'done',
      ...(nextSection && { [nextSection]: 'active' }),
    }));

    // If this was the last job section, mark Pod as active
    if (!nextSection) {
      setPodStatus('done');
  ...
  // Handle edit
  const handleEdit = useCallback((section: JobSectionStep) => {
    setEditingSection(section);
    const sectionIndex = JOB_SECTION_ORDER.indexOf(section);

    setJobSectionStatus((prev) => {
      const newStatus = { ...prev };
      JOB_SECTION_ORDER.forEach((key, index) => {
  ...
  const handleEditCancel = useCallback(() => {
    ...
    setJobSectionStatus((prev) => {
      ...
      const editIndex = JOB_SECTION_ORDER.indexOf(editingSection);
      for (let i = editIndex + 1; i < JOB_SECTION_ORDER.length; i++) {
        if (newStatus[JOB_SECTION_ORDER[i]] === 'writing') {
          newStatus[JOB_SECTION_ORDER[i]] = 'done';
        }
      }
      return newStatus;
    });
  ...
  const allJobSectionsDone = JOB_SECTION_ORDER.every((key) => jobSectionStatus[key] === 'done');
  const podDone = podStatus === 'done';
  const allContainersDone = containerStatuses.every((c) => c.status === 'done');
  const isCreateDisabled = !allJobSectionsDone || !podDone || !allContainersDone;
          ...
          <div className="pt-4 px-8 pb-20 bg-[var(--color-surface-default)]">
            <VStack gap={6}>
              ...
              <HStack gap={6} align="start" className="w-full">
                ...
                <VStack gap={4} className="flex-1">
                  {activeTab === 'job' && (
                    <>
                      ...
                      {jobSectionStatus['basic-info'] === 'pre' && (
                        <PreSection title={JOB_SECTION_LABELS['basic-info']} />
                      )}
                      {jobSectionStatus['basic-info'] === 'writing' && (
                        <WritingSection title={JOB_SECTION_LABELS['basic-info']} />
                      )}
                      {jobSectionStatus['basic-info'] === 'active' && (
                        <BasicInfoSection
                          namespace={namespace}
                      ...
                      {jobSectionStatus['basic-info'] === 'done' && (
                        <DoneSection
                          title={JOB_SECTION_LABELS['basic-info']}
                      ...
                      {jobSectionStatus['labels-annotations'] === 'pre' && (
                        <PreSection title={JOB_SECTION_LABELS['labels-annotations']} />
                      )}
                      {jobSectionStatus['labels-annotations'] === 'writing' && (
                        <WritingSection title={JOB_SECTION_LABELS['labels-annotations']} />
                      )}
                      {jobSectionStatus['labels-annotations'] === 'active' && (
                      ...
                      {jobSectionStatus['labels-annotations'] === 'done' && (
                        <DoneSection
                          title={JOB_SECTION_LABELS['labels-annotations']}
                      ...
                      {jobSectionStatus['scaling-policy'] === 'pre' && (
                        <PreSection title={JOB_SECTION_LABELS['scaling-policy']} />
                      )}
                      {jobSectionStatus['scaling-policy'] === 'writing' && (
                        <WritingSection title={JOB_SECTION_LABELS['scaling-policy']} />
                      )}
                      {jobSectionStatus['scaling-policy'] === 'active' && (
                      ...
                      {jobSectionStatus['scaling-policy'] === 'done' && (
                        <DoneSection
                          title={JOB_SECTION_LABELS['scaling-policy']}
                ...
                {/* Summary Sidebar */}
                <SummarySidebar
                  jobSectionStatus={jobSectionStatus}
