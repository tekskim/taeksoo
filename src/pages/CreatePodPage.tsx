...
type PodConfigSectionStep = 'basic-info' | 'labels-annotations' | 'scaling-policy';
...
const POD_CONFIG_SECTION_LABELS: Record<PodConfigSectionStep, string> = {
...
const POD_CONFIG_SECTION_ORDER: PodConfigSectionStep[] = [
  'basic-info',
  'labels-annotations',
  'scaling-policy',
];
...
interface SummarySidebarProps {
  podConfigSectionStatus: Record<PodConfigSectionStep, SectionState>;
...
function SummarySidebar({ 
  ...
  const [podConfigExpanded, setPodConfigExpanded] = useState(true);
  ...
  // Check if all pod config sections are done
  const allPodConfigDone = POD_CONFIG_SECTION_ORDER.every(
    (key) => podConfigSectionStatus[key] === 'done'
  );
          ...
          <VStack gap={3}>
            ...
            <VStack gap={0}>
              ...
              >
                ...
                {allPodConfigDone && <WizardSectionStatusIcon status="done" />}
              </HStack>

              {/* Pod Sub-items (Indented) */}
              {podConfigExpanded && (
                <VStack gap={0} className="pl-4">
                  {POD_CONFIG_SECTION_ORDER.map((key) => {
...
export function CreatePodPage() {
  ...
  // Active tab (Pod Config, Pod, Container-X)
  ...
  // Section states for Pod Config tab
  const [podConfigSectionStatus, setPodConfigSectionStatus] = useState<Record<PodConfigSectionStep, SectionState>>({
    'basic-info': 'active',
  ...
  // Editing state
  const [editingSection, setEditingSection] = useState<PodConfigSectionStep | null>(null);
  ...
  const handleNext = useCallback((currentSection: PodConfigSectionStep) => {
    const currentIndex = POD_CONFIG_SECTION_ORDER.indexOf(currentSection);
    const nextSection = POD_CONFIG_SECTION_ORDER[currentIndex + 1];

    setPodConfigSectionStatus((prev) => ({
      ...prev,
      [currentSection]: 'done',
      ...(nextSection && { [nextSection]: 'active' }),
    }));

    // If this was the last pod config section, mark Pod as active
    if (!nextSection) {
      setPodStatus('done');
      setContainerStatuses((prev) => 
  ...
  // Handle edit
  const handleEdit = useCallback((section: PodConfigSectionStep) => {
    setEditingSection(section);
    const sectionIndex = POD_CONFIG_SECTION_ORDER.indexOf(section);

    setPodConfigSectionStatus((prev) => {
      const newStatus = { ...prev };
      POD_CONFIG_SECTION_ORDER.forEach((key, index) => {
  ...
  const handleEditCancel = useCallback(() => {
    ...
    setPodConfigSectionStatus((prev) => {
      ...
      const editIndex = POD_CONFIG_SECTION_ORDER.indexOf(editingSection);
      for (let i = editIndex + 1; i < POD_CONFIG_SECTION_ORDER.length; i++) {
        if (newStatus[POD_CONFIG_SECTION_ORDER[i]] === 'writing') {
          newStatus[POD_CONFIG_SECTION_ORDER[i]] = 'done';
        }
      }
      return newStatus;
    });
  ...
  const allPodConfigSectionsDone = POD_CONFIG_SECTION_ORDER.every(
    (key) => podConfigSectionStatus[key] === 'done'
  );
  const podDone = podStatus === 'done';
  const allContainersDone = containerStatuses.every((c) => c.status === 'done');
  const isCreateDisabled = !allPodConfigSectionsDone || !podDone || !allContainersDone;
          ...
          <div className="pt-4 px-8 pb-20 bg-[var(--color-surface-default)]">
            <VStack gap={6}>
              ...
              <HStack gap={6} align="start" className="w-full">
                ...
                <VStack gap={4} className="flex-1">
                  {activeTab === 'pod-config' && (
                    <>
                      ...
                      {podConfigSectionStatus['basic-info'] === 'pre' && (
                        <PreSection title={POD_CONFIG_SECTION_LABELS['basic-info']} />
                      )}
                      {podConfigSectionStatus['basic-info'] === 'writing' && (
                        <WritingSection title={POD_CONFIG_SECTION_LABELS['basic-info']} />
                      )}
                      {podConfigSectionStatus['basic-info'] === 'active' && (
                        <BasicInfoSection
                          namespace={namespace}
                      ...
                      {podConfigSectionStatus['basic-info'] === 'done' && (
                        <DoneSection
                          title={POD_CONFIG_SECTION_LABELS['basic-info']}
                      ...
                      {podConfigSectionStatus['labels-annotations'] === 'pre' && (
                        <PreSection title={POD_CONFIG_SECTION_LABELS['labels-annotations']} />
                      )}
                      {podConfigSectionStatus['labels-annotations'] === 'writing' && (
                        <WritingSection title={POD_CONFIG_SECTION_LABELS['labels-annotations']} />
                      )}
                      {podConfigSectionStatus['labels-annotations'] === 'active' && (
                      ...
                      {podConfigSectionStatus['labels-annotations'] === 'done' && (
                        <DoneSection
                          title={POD_CONFIG_SECTION_LABELS['labels-annotations']}
                      ...
                      {podConfigSectionStatus['scaling-policy'] === 'pre' && (
                        <PreSection title={POD_CONFIG_SECTION_LABELS['scaling-policy']} />
                      )}
                      {podConfigSectionStatus['scaling-policy'] === 'writing' && (
                        <WritingSection title={POD_CONFIG_SECTION_LABELS['scaling-policy']} />
                      )}
                      {podConfigSectionStatus['scaling-policy'] === 'active' && (
                      ...
                      {podConfigSectionStatus['scaling-policy'] === 'done' && (
                        <DoneSection
                          title={POD_CONFIG_SECTION_LABELS['scaling-policy']}
                ...
                {/* Summary Sidebar */}
                <SummarySidebar
                  podConfigSectionStatus={podConfigSectionStatus}
