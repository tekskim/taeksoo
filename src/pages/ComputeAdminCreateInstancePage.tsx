                  ...
                  <VStack gap={2}>
                    ...
                    <SelectionIndicator
                      selectedItems={mockExistingFloatingIPs
                        ...
                        .map((f) => ({ id: f.id, label: `${f.id}(${f.description})` }))}
                      onRemove={(id) => {
                        const newSet = new Set(selectedExistingFip);
                        newSet.delete(id);
                        setSelectedExistingFip(newSet);
    ...
    <SectionCard isActive={isActive}>
      ...
      <SectionCard.Content showDividers={false}>
        <VStack gap={0}>
            ...
            <VStack gap={3} align="start" className="w-full">
              <VStack gap={1}>
                ...
                <span className="text-body-md text-[var(--color-text-subtle)]">
                  ...
                  predefined settings such as the image, flavor, and network configuration required
                  for the instance.
                </span>
              </VStack>
          ...
          {!isEditing && (
            <HStack gap={2} justify="end" className="pt-3 w-full">
...
export function ComputeAdminCreateInstancePage() {
  ...
  // Handle cancel
  const handleCancel = () => {
    navigate('/compute-admin/instances');
  };
      ...
      <VStack gap={3} className="min-w-[1176px]">
        ...
        <HStack gap={6} align="start" className="w-full">
          ...
          <VStack gap={4} className="flex-1">
            {/* Templates Section */}
            {!isV2 && sectionStatus.templates === 'pre' && (
              <PreSection title={SECTION_LABELS.templates} />
            )}
            {!isV2 && sectionStatus.templates === 'writing' && (
              <WritingSection title={SECTION_LABELS.templates} />
            )}
            {(isV2 || sectionStatus.templates === 'active') && (
              <TemplatesSection
            ...
            {!isV2 && sectionStatus.templates === 'skipped' && (
              <SkippedSection
            ...
            {!isV2 && sectionStatus.authentication === 'pre' && (
              <PreSection title={SECTION_LABELS.authentication} />
            )}
            {!isV2 && sectionStatus.authentication === 'writing' && (
              <WritingSection title={SECTION_LABELS.authentication} />
            )}
            {(isV2 || sectionStatus.authentication === 'active') && (
...
export default ComputeAdminCreateInstancePage;
