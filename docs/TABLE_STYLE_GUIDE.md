...
3. [행 높이 정책](#3-행-높이-정책)
4. [텍스트 처리 정책](#4-텍스트-처리-정책)
5. [마이그레이션 가이드](#5-마이그레이션-가이드)
6. [통합 코드 예시](#6-통합-코드-예시)
7. [FAQ](#7-faq)
...
| `locked` | 68px | 잠금 아이콘 | `IconLock` |
| `lockedWide` | 70px | 잠금 아이콘 (넓은 버전) | - |
| `actions` | 64px | 액션 메뉴 | `ContextMenu` |
| `actionWide` | 72px | 액션 메뉴 (넓은 버전) | - |
| `actionXl` | 80px | 액션 메뉴 (XL 버전) | - |
| `mfa` | 80px | MFA 아이콘 | MFA 상태 표시 |
...
// 상세 페이지 탭 - 40px
<Table columns={attachedVolumesColumns} data={volumes} rowKey="id" rowHeight="40px" />

// 모달 내 테이블 - 40px
<Modal open={isOpen}>
  <Modal.Content>
    <Table columns={selectColumns} data={items} rowKey="id" rowHeight="40px" />
  </Modal.Content>
</Modal>
```
...
| 원칙 | 설명 |
|------|------|
| **단일행 유지** | 테이블 셀은 항상 한 줄로 표시 |
| **Truncate 적용** | 긴 텍스트는 말줄임표(...)로 잘림 |
...
const columns: TableColumn<Instance>[] = [
  ...
  // 유연: IP 주소
  {
    ...
    render: (_, row) => (
      <span className="font-mono truncate" title={row.ip}>{row.ip}</span>
    ),
  },
  // 유연: 생성일 (줄바꿈 방지)
  {
...
```typescript
// 밀집 높이 (40px) 사용
const attachedVolumesColumns: TableColumn<Volume>[] = [
  {
    key: 'name',
    label: 'Name',
    flex: 1,
  ...
  },
  {
    ...
    align: 'center',
    render: (_, row) => (
      <Button variant="ghost" size="sm" onClick={() => handleDetach(row.id)}>
        Detach
      </Button>
    ),
  },
...
<Table 
  columns={attachedVolumesColumns} 
  data={volumes} 
  rowKey="id" 
  rowHeight="40px"  // 밀집 레이아웃
/>
```
...
const detailedColumns: TableColumn<Instance>[] = [
  ...
  { key: 'locked', width: fixedColumns.locked, align: 'center', /* ... */ },
  { key: 'fixedIp', flex: 1, minWidth: columnMinWidths.fixedIp, /* ... */ },
  { key: 'floatingIp', flex: 1, minWidth: columnMinWidths.floatingIp, /* ... */ },
  { key: 'image', flex: 1, minWidth: columnMinWidths.image, /* ... */ },
  { key: 'flavor', flex: 1, minWidth: columnMinWidths.flavor, /* ... */ },
  { key: 'vcpu', flex: 1, minWidth: columnMinWidths.vcpu, /* ... */ },
  { key: 'ram', flex: 1, minWidth: columnMinWidths.ram, /* ... */ },
  { key: 'disk', flex: 1, minWidth: columnMinWidths.disk, /* ... */ },
