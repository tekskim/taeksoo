"use strict";
(self["webpackChunk_thaki_tds"] = self["webpackChunk_thaki_tds"] || []).push([["src_pages_kms_mocks_cryptoKeysRepository_ts"], {
"./src/pages/kms/mocks/cryptoKeysRepository.ts"(__unused_rspack_module, __webpack_exports__, __webpack_require__) {
__webpack_require__.r(__webpack_exports__);
__webpack_require__.d(__webpack_exports__, {
  getMockCryptoKeyBySlug: () => (getMockCryptoKeyBySlug),
  listMockCryptoKeys: () => (listMockCryptoKeys),
  resetMockCryptoKeys: () => (resetMockCryptoKeys),
  rotateMockCryptoKey: () => (rotateMockCryptoKey),
  setMockCryptoKeys: () => (setMockCryptoKeys),
  setMockCryptoKeysDelay: () => (setMockCryptoKeysDelay),
  updateMockCryptoKeyStatus: () => (updateMockCryptoKeyStatus)
});
const DEFAULT_DELAY_MS = 80;
const MOCK_CRYPTO_KEYS = [
    {
...
const MOCK_CRYPTO_KEY_DETAILS = [
    {
        ...
        rotationHistory: [
            ...
            {
                version: 12,
                rotatedAt: '2026-01-30T23:52:51Z',
                status: 'deactivated'
            },
            {
                version: 11,
                rotatedAt: '2025-11-01T23:52:51Z',
                status: 'deactivated'
            },
            {
                version: 10,
                rotatedAt: '2025-08-03T23:52:51Z',
                status: 'deactivated'
            },
            {
                version: 9,
                rotatedAt: '2025-05-05T23:52:51Z',
                status: 'deactivated'
            },
            {
                version: 8,
                rotatedAt: '2025-02-04T23:52:51Z',
                status: 'archived'
            },
            {
                version: 7,
                rotatedAt: '2024-11-06T23:52:51Z',
                status: 'archived'
            },
            {
                version: 6,
                rotatedAt: '2024-08-08T23:52:51Z',
                status: 'archived'
            },
            {
                version: 5,
                rotatedAt: '2024-05-10T23:52:51Z',
                status: 'archived'
            },
            {
                version: 4,
                rotatedAt: '2024-02-10T23:52:51Z',
                status: 'destroyed'
            },
            {
                version: 3,
                rotatedAt: '2023-11-12T23:52:51Z',
                status: 'destroyed'
            },
            {
                version: 2,
                rotatedAt: '2023-08-14T23:52:51Z',
                status: 'destroyed'
            },
            {
                version: 1,
                rotatedAt: '2023-05-16T23:52:51Z',
                status: 'destroyed'
            }
        ]
    },
...
let currentKeyDetails = [
    ...MOCK_CRYPTO_KEY_DETAILS
];
...
const matchesArrayFilter = (value, selected)=>{
    if (!selected || selected.length === 0) return true;
    return selected.includes(value);
};
const listMockCryptoKeys = async (params)=>{
    await delay(responseDelayMs);
    const filtered = currentKeys.filter((item)=>{
        const matchesSearch = !params.search || [
            item.name,
            item.algorithm,
            item.purpose
        ].some((value)=>includesAny(value, params.search));
        return matchesSearch && matchesArrayFilter(item.algorithm, params.algorithms) && matchesArrayFilter(item.purpose, params.purposes) && matchesArrayFilter(item.status, params.statuses);
    }).sort((left, right)=>{
...
const getMockCryptoKeyBySlug = async (slug)=>{
    await delay(responseDelayMs);
    return currentKeyDetails.find((item)=>item.slug === slug) ?? null;
};
const resetMockCryptoKeys = ()=>{
    currentKeys = [
        ...MOCK_CRYPTO_KEYS
    ];
    currentKeyDetails = [
...
const updateMockCryptoKeyStatus = async (slug, status)=>{
    await delay(responseDelayMs);
    currentKeys = currentKeys.map((k)=>k.slug === slug ? {
            ...k,
            status
        } : k);
    currentKeyDetails = currentKeyDetails.map((d)=>d.slug === slug ? {
            ...d,
            status
        } : d);
};
const rotateMockCryptoKey = async (slug)=>{
    await delay(responseDelayMs);
    const now = new Date().toISOString();
    currentKeyDetails = currentKeyDetails.map((d)=>{
        var _d_rotationHistory_;
        if (d.slug !== slug) return d;
        const nextVersion = (((_d_rotationHistory_ = d.rotationHistory[0]) === null || _d_rotationHistory_ === void 0 ? void 0 : _d_rotationHistory_.version) ?? 0) + 1;
        const updatedHistory = d.rotationHistory.map((h)=>h.status === 'active' ? {
                ...h,
                status: 'deactivated'
            } : h);
        return {
...
const setMockCryptoKeysDelay = (ms)=>{
    responseDelayMs = ms;
};
...
//# sourceMappingURL=src_pages_kms_mocks_cryptoKeysRepository_ts.c0b98bd81f06e702.js.map